...
const W={
  ...
  _drafts:{},_toastCid:null,_newCount:0,
  ...
  addMessage(cid,msg){
    ...
    this.renderChats();
    // Compteur de nouveaux messages (style WhatsApp) : si l'utilisateur
    // lit l'historique (scrollé vers le haut), on compte les entrants
    // et le bouton ↓ affiche le total au lieu d'être muet.
    if(cid===this.activeId && msg.from!=='me' && this._userScrolled){this._newCount++;this._updNewBadge()}
  ...
  openChat(cid){
    this._stopVoice();
    // Brouillons (confort WhatsApp) : on garde le texte en cours dans la
    // conversation quittée, et on ne laisse pas une bannière « répondre /
    // modifier » d'une autre conversation pointer vers l'ancienne.
    if (this.activeId && this.activeId !== cid) {
      const pt=$('waTxt');
      if (pt) this._drafts[this.activeId]=pt.value;
      if (this.editId) this.cancelEdit(); else if (this.replyTo) this.cancelReply();
    }
    ...
    // Brouillon de cette conversation restauré + focus automatique du champ
    // (ouvre le clavier sur mobile, comme WhatsApp).
    const t=$('waTxt');
    if(t){t.value=this._drafts[cid]||'';this.autoResize(t);setTimeout(()=>{try{t.focus()}catch(e){}},180)}
    this._userScrolled=false;this._newCount=0;this._updNewBadge();this.emit('chatOpened',cid);
  },
  // Affiche/masque le compteur de nouveaux messages sur le bouton ↓
  _updNewBadge(){const b=$('waScrollBadge');if(!b)return;if(this._newCount>0){b.textContent=this._newCount>99?'99+':this._newCount;b.style.display='flex'}else{b.style.display='none'}},
  goBack(){
    this._stopVoice();this._exitSel();
    if (this.activeId) { const t=$('waTxt'); if (t) this._drafts[this.activeId]=t.value; }
  ...
  renderChats(){
    ...
    el.innerHTML=list.map(c=>{
      ...
      if(last){
        ...
        else if(last.image||(last.images&&last.images.length))lastH+=`<span>📷 ${T('photo')}</span>`;
  ...
  renderMsgs(){
    ...
    ms.forEach((m,i)=>{
      ...
      if(m.replyTo){const rm=ms.find(x=>x.id===m.replyTo);if(rm){const rn=rm.from==='me'?T('you'):((this.contacts.find(c=>c.id===rm.from)||{name:T('unknown')}).name);h+=`<div class="wa-quote" onclick="W.scrollToMsg('${m.replyTo}')"><div class="wa-quote-name">${esc(rn)}</div><div class="wa-quote-text">${rm.text?esc(rm.text):(rm.image||(rm.images&&rm.images.length))?'📷 '+T('photo'):rm.voice?'🎤 '+T('voice'):'📎 '+T('file')}</div></div>`}}
      if(m.poll){h+=this._renderPoll(m)}
      if(m.image)h+=`<div class="wa-img" onclick="W.openLB('${m.imageFull||m.image}')"><img src="${m.image}" alt="" loading="lazy"></div>`;
      if(m.images&&m.images.length)h+=`<div class="wa-imgs">${m.images.map((u,i)=>`<div class="wa-img" onclick="W.openLB('${(m.imagesFull&&m.imagesFull[i])||u}')"><img src="${u}" alt="" loading="lazy"></div>`).join('')}</div>`;
      ...
      if(m.autoReply)h+=`<div class="wa-autoreply">🤖 ${T('autoReply')||'Réponse automatique'}</div>`;
      h+=`<div class="wa-meta">`;
  ...
  scrollToBottom(force){const el=$('waMsgs');if(force||!this._userScrolled){requestAnimationFrame(()=>{el.scrollTop=el.scrollHeight});this._userScrolled=false;this._newCount=0;this._updNewBadge();$('waScrollBtn').classList.remove('visible')}},
  ...
  _showToast(cid,msg){const c=this.contacts.find(c=>c.id===cid);if(!c)return;this._toastCid=cid;const t=$('waToast');$('waToastAv').src=c.avatar;$('waToastName').textContent=c.name;$('waToastText').textContent=(msg.text||('📷 '+T('photo'))).slice(0,60);t.onclick=()=>this.toastTap();t.classList.add('active');clearTimeout(this._toastT);this._toastT=setTimeout(()=>t.classList.remove('active'),4000)},
  // Toast cliquable (confort WhatsApp) : tap sur la notification interne →
  // ouvre directement la conversation concernée.
  toastTap(){const t=$('waToast');if(!t||!t.classList.contains('active'))return;const cid=this._toastCid;this._toastCid=null;clearTimeout(this._toastT);t.classList.remove('active');if(cid)this.openChat(cid)},
  ...
  _send(){
    ...
    if(this.editId){this.editMessage(cid,this.editId,text);this.cancelEdit();t.value='';t.style.height='auto';this._drafts[cid]='';this._updSend();return}
    const data={text};
    ...
    this.sendMessage(cid,data);t.value='';t.style.height='auto';this._drafts[cid]='';this._updSend();
    this.emit('messageSent',{contactId:cid,text});
  },
  ...
  onFile(inp){const files=[...(inp.files||[])];if(!files.length||!this.activeId)return;const cid=this.activeId;const read=f=>new Promise(r=>{const rd=new FileReader();rd.onload=e=>r(e.target.result);rd.readAsDataURL(f)});Promise.all(files.filter(f=>f.type&&f.type.startsWith('image/')).slice(0,6).map(read)).then(urls=>{if(!urls.length)return;if(urls.length===1)this.sendMessage(cid,{image:urls[0]});else this.sendMessage(cid,{images:urls})}).catch(()=>{});files.filter(f=>!f.type||!f.type.startsWith('image/')).forEach(f=>this.sendMessage(cid,{file:{name:f.name,size:fmtSize(f.size)}}));inp.value=''},
  ...
  _renderProfile(){
    ...
    $('waProfAv').src=c.avatar;$('waProfName').textContent=c.name+(c.profileType==='company'?' \uD83C\uDFE2':'');
    $('waProfStatus').textContent=c.online?T('online'):(c.lastSeen?T('lastSeen',c.lastSeen):'');
    $('waProfAbout').textContent=c.about||T('about');
    const ms=this.convs[this.activeId]||[];const imgs=ms.flatMap(m=>{const full=[];(m.imagesFull||m.images||[]).forEach((u,i)=>full.push((m.imagesFull&&m.imagesFull[i])||u));if(m.imageFull||m.image)full.push(m.imageFull||m.image);return full.filter(Boolean);});
    $('waProfMedia').innerHTML=imgs.map(u=>`<img src="${u}" alt="" onclick="W.openLB('${u}')">`).join('')||'<p style="color:var(--text-secondary);font-size:13px;grid-column:1/-1">'+T('noMedia')+'</p>';
  },

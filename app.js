...
// ===== LOGGER D'ERREURS CÔTÉ CLIENT (anti-boucle) =====
// Limites volontaires : MAX 10 logs par session, la même erreur 1 fois /
// 30 s, et le logger ne s'enregistre JAMAIS lui-même (une écriture
// errorLogs refusée — ex. non connecté — ne doit jamais créer une nouvelle
// erreur loggable, c'est ça qui créait la boucle de 99+ permission_denied).
const errorLogState = { count: 0, byMsg: {} };
try { window.__errorLogState = errorLogState; } catch (e) {} // tests jsdom
function logClientError(message, extra) {
  try {
    const msg = String(message || '').slice(0, 300);
    // Anti-boucle : les échecs du logger lui-même (errorLogs) ne sont
    // jamais loggés — sinon : erreur → log refusé → nouvelle erreur →
    // log refusé → ... (la console se remplissait de 99+ erreurs).
    if (/errorLogs/i.test(msg)) return false;
    if (errorLogState.count >= 10) return false;
    const now = Date.now();
    const last = errorLogState.byMsg[msg] || 0;
...
let allJobs = [];
// Alias « vivant » sur window (tests automatisés + console de debug).
try { Object.defineProperty(window, 'allJobs', { configurable: true, get: () => allJobs, set: (v) => { allJobs = v; } }); } catch (e) {}
...
// Alias « vivant » sur window (tests automatisés + console de debug) : le
// getter suit les réaffectations de la variable (syncProfilesCache la remplace
// au 1er chargement puis la met à jour par profil).
try { Object.defineProperty(window, 'profilesCache', { configurable: true, get: () => profilesCache, set: (v) => { profilesCache = v; } }); } catch (e) {}
...
function syncProfilesCache() {
  ...
  // NB : les 3 listeners relançant le même « liste + popups », on passe par
  // scheduleJobsRefresh() (debounce 150 ms) : les mises à jour de profil
  // arrivent en rafale (notes recalculées après chaque avis) et chaque tick
  // ne doit pas déclencher un re-rendu complet.
  ...
  profilesQuery.on('child_changed', snap => {
    ...
    // données de profil — refreshAllPopupsLanguage() est appelé dans le
    // même scheduleJobsRefresh() que la liste.
    scheduleJobsRefresh();
...
function initOfflineDetection() {
  ...
  let prevOffline = !navigator.onLine;
  const update = () => {
    const isOffline = !navigator.onLine;
    if (banner) banner.style.transform = isOffline ? 'translateY(0)' : 'translateY(-100%)';
    if (isOffline) {
      // Popup UNE fois par épisode hors-ligne (pas relancé après « Plus tard »
      // tant que la connexion n'est pas revenue). Les tuiles des zones déjà
      // visitées sont en cache → le plan de la carte reste utilisable.
      if (!popupShown) { showOfflinePopup(); popupShown = true; }
    } else if (prevOffline) {
      hideOfflinePopup();
      popupShown = false;
...
// CTA du popup hors-ligne : bascule sur la vue CARTE — le seul mode vraiment
// utilisable sans réseau (tuiles déjà chargées = cache navigateur).
function offlinePopupMap() {
  hideOfflinePopup();
  try { activateNav('map'); } catch (e) {}
}
...
const I18N = {
  fr: {
    ...
    statRevenueSub: "sur {n} annonce(s) pourvue(s)",
    ...
    installManualAndroid: "Android : menu ⋮ du navigateur → « Ajouter sur l’écran d’accueil » (ou « Installer l’app »)",
    installManualIos: "iPhone/iPad : bouton Partager (carré avec flèche vers le haut) → « Sur l’écran d’accueil »",
    installManualNote: "Une fois sur l’écran d’accueil, l’app se met à jour toute seule à chaque ouverture — pas besoin de repasser par un store.",
    okBtn: "OK",
    ...
    completenessComplete: "✅ Profil complet — tu es prêt à recevoir plus de contacts !", autoReply: "Réponse automatique", aiAwayFallback: "Bonjour, c'est {name} 👋 J'ai bien reçu votre message et je vous réponds dès que possible.", portfolioTitle: "📸 Mes réalisations", portfolioSub: "Montre tes plus beaux travaux (jusqu'à 6 photos). Les clients font plus confiance à un artisan qui montre son travail.", portfolioPublicTitle: "📸 Ses réalisations", portfolioPhotosCount: "{n} photo(s)", aiSuggestBtn: "✨ Répondre avec l’IA", guaranteeTitle: "🛡️ Garantie JobMarket", guaranteeLine1: "Si un problème survient avec ce prestataire, notre équipe intervient pour médier.", guaranteeLine2: "Règle d’or : ne payez JAMAIS l’intégralité à l’avance — soldez après le travail terminé.", aiSuggestLoading: "✨ Génération…", aiSuggestFallback: "Bonjour, j’ai bien reçu votre message. Je suis dispo cette semaine et je reviens vers vous très vite pour les détails.",
    availFilterEmptyTitle: "Personne n’est « dispo maintenant » ici",
    availFilterEmptySub: "Désactive le filtre 🟢 (touche au bouton « Dispo ») pour voir toutes les annonces.",
    availFilterOnToast: "🟢 Filtre « Dispo » activé : {n} annonce(s) de personnes disponibles maintenant.",
    availFilterEmptyToast: "Personne n’est « dispo maintenant » dans cette zone. Touche à nouveau le bouton 🟢 pour tout voir.",
    availFilterOffToast: "Filtre « Dispo » désactivé : toutes les annonces sont de nouveau affichées.",
    ...
    offlinePopTitle: "Vous êtes hors ligne", offlinePopBody: "Pas de connexion internet. La carte reste consultable sur les zones déjà chargées — les actions en ligne (publier, contacter, chat) reprendront au retour du réseau.", offlinePopMapBtn: "Voir le plan de la carte", offlinePopLater: "Plus tard", backOnline: "De retour en ligne ✅",
    ...
    newJobToast: "🎉 Nouveau job : {title} ({location}) vient d'être publié !", smartJobToast: "🔔 Nouveau job pour toi : {title} ({location})", invalidPhotoFormat: "Photo invalide : JPG/PNG/WEBP, max {mb} Mo.", positionSavedComplete: "Position enregistrée ! Complétez le formulaire.",
 ...
 newMsgs: "Nouveaux messages",
    ...
    jobsNearby: "Jobs proches", sortLabel: "Trier :", sortNearest: "Le plus proche", sortRating: "Mieux notés", sortPrice: "Moins cher", sortNewest: "Plus récent", sortProFirst: "⭐ Pro / vérifié d'abord", emptyTitle: "Aucun job trouve", emptySub: "Publiez le premier job dans cette zone",
    ...
    adminDashboard: "Dashboard Admin", secureChat: "Chat sécurisé", myListings: "Mes publications", myProProfile: "Mon Profil Pro", profileTypeLabel: "Type de profil", profileTypePerson: "Personne", profileTypeCompany: "Entreprise / Organisation", profileTypeCompanyHint: "Le nom de la société sera mis en avant avec le badge 🏢.",
    ...
    verificationCodeExpired: "Code expiré (valable 24 h) — contacte-nous pour une nouvelle vérification.",
  ...
  en: {
    ...
    statRevenueSub: "across {n} filled listing(s)",
    ...
    installManualAndroid: "Android: browser menu ⋮ → “Add to home screen” (or “Install app”)",
    installManualIos: "iPhone/iPad: Share button (square with up arrow) → “Add to Home Screen”",
    installManualNote: "Once on your home screen, the app updates itself on every open — no store needed.",
    okBtn: "OK",
    ...
    completenessComplete: "✅ Profile complete — ready for more contacts!", autoReply: "Automatic reply", aiAwayFallback: "Hi, this is {name} 👋 I got your message and I'll reply as soon as possible.", portfolioTitle: "📸 My work", portfolioSub: "Show your best work (up to 6 photos). Clients trust a tradesperson who shows their work.", portfolioPublicTitle: "📸 Their work", portfolioPhotosCount: "{n} photo(s)", aiSuggestBtn: "✨ AI reply", guaranteeTitle: "🛡️ JobMarket Guarantee", guaranteeLine1: "If a problem occurs with this provider, our team steps in to mediate.", guaranteeLine2: "Golden rule: NEVER pay the full amount upfront — settle after the work is done.", aiSuggestLoading: "✨ Generating…", aiSuggestFallback: "Hi, I got your message. I’m available this week and I’ll get back to you quickly with the details.",
    availFilterEmptyTitle: "No one is « available now » here",
    availFilterEmptySub: "Turn off the 🟢 filter (tap the « Dispo » button) to see all listings.",
    availFilterOnToast: "🟢 « Available now » filter on: {n} listing(s) from people available now.",
    availFilterEmptyToast: "No one is « available now » in this area. Tap the 🟢 button again to see everything.",
    availFilterOffToast: "« Available now » filter off: all listings are shown again.",
    ...
    offlinePopTitle: "You're offline", offlinePopBody: "No internet connection. The map is still available for areas already loaded — online actions (post, contact, chat) resume when you're back.", offlinePopMapBtn: "See the map", offlinePopLater: "Later", backOnline: "Back online ✅",
    ...
    newJobToast: "🎉 New job: {title} ({location}) was just posted!", smartJobToast: "🔔 New job for you: {title} ({location})", invalidPhotoFormat: "Invalid photo: JPG/PNG/WEBP, max {mb} MB.", positionSavedComplete: "Position saved! Complete the form.",
 ...
 newMsgs: "New messages",
    ...
    jobsNearby: "Nearby jobs", sortLabel: "Sort by", sortNearest: "Nearest", sortRating: "Top rated", sortPrice: "Cheapest", sortNewest: "Newest", sortProFirst: "⭐ Pro / verified first", emptyTitle: "No jobs found", emptySub: "Post the first job in this area",
    ...
    adminDashboard: "Admin Dashboard", secureChat: "Secure chat", myListings: "My listings", myProProfile: "My Pro Profile", profileTypeLabel: "Profile type", profileTypePerson: "Person", profileTypeCompany: "Company / Organization", profileTypeCompanyHint: "The company name will be highlighted with the 🏢 badge.",
    ...
    verificationCodeExpired: "Code expired (valid 24 h) — contact us for a new verification.",
  ...
  it: {
    qrPriceContext: "Quanto costa? (annuncio: {price})",
    qrPriceFree: "È gratis?",
    qrMissionThanks: "🙏 Grazie per il lavoro",
    qrRecommend: "⭐ Ti consiglio",
    specialtiesLabel: "Specializzazioni",

    statRevenueLabel: "Guadagni stimati",
    statRevenueLocked: "Passa a PRO per seguire i tuoi guadagni stimati.",
    statRevenueSub: "su {n} annunci coperti",
    installSettingsBtn: "Installa l’app",
    installSettingsInstalled: "App installata ✅",
    installManualTitle: "Installa JobMarket sul tuo telefono",
    installManualAndroid: "Android: menu del browser ⋮ → “Aggiungi a schermata Home” (o “Installa app”)",
    installManualIos: "iPhone/iPad: pulsante Condivisione (quadrato con freccia su) → “Su schermata Home”",
    installManualNote: "Una volta sulla schermata Home, l’app si aggiorna da sola ad ogni apertura — nessun store.",
    okBtn: "OK",
    qualLabel: "Qualifica (opzionale)",
    qualNone: "—",
    qualAuto: "Autodidatta / Esperienza",
    qualCap: "Attestato professionale / Formazione pratica",
    qualBts: "Diploma tecnico (BTS) / Qualificato",
    qualIng: "Ingegnere / Esperto certificato",
    completenessTitle: "Completamento del profilo",
    completenessMissing: "Da completare:",
    completenessComplete: "✅ Profilo completo — pronto per più contatti!", autoReply: "Risposta automatica", aiAwayFallback: "Ciao, sono {name} 👋 Ho ricevuto il tuo messaggio e ti risponderò il prima possibile.", portfolioTitle: "📸 I miei lavori", portfolioSub: "Mostra i tuoi migliori lavori (fino a 6 foto). I clienti fidano di più di un artigiano che mostra il suo lavoro.", portfolioPublicTitle: "📸 I suoi lavori", portfolioPhotosCount: "{n} foto", aiSuggestBtn: "✨ Rispondi con IA", guaranteeTitle: "🛡️ Garanzia JobMarket", guaranteeLine1: "Se si verificano problemi con questo fornitore, il nostro team interviene per mediare.", guaranteeLine2: "Regola d’oro: NON pagare mai l’intero importo in anticipo — salda dopo il lavoro completato.", aiSuggestLoading: "✨ Generazione…", aiSuggestFallback: "Ciao, ho ricevuto il tuo messaggio. Sono disponibile questa settimana e ti rispondo presto con i dettagli.",
    availFilterEmptyTitle: "Nessuno è « disponibile ora » qui",
    availFilterEmptySub: "Disattiva il filtro 🟢 (tocca il pulsante « Dispo ») per vedere tutti gli annunci.",
    availFilterOnToast: "🟢 Filtro « Disponibile ora » attivo: {n} annunci di persone disponibili.",
    availFilterEmptyToast: "Nessuno è « disponibile ora » in questa zona. Torna a toccare il pulsante 🟢 per vedere tutto.",
    availFilterOffToast: "Filtro « Disponibile ora » disattivato: tutti gli annunci sono di nuovo visibili.",
    trustSignalsTitle: "🛡️ Segnali di fiducia",
    statRatingLabel: "Recensioni",
    statMissionsLabel: "Missioni",
    statConfirmedLabel: "Confermate",
    statReactivityLabel: "Tempo di risposta",
    statStatusLabel: "Stato",
    ...
    offlinePopTitle: "Sei offline", offlinePopBody: "Nessuna connessione internet. La mappa resta consultabile nelle zone già caricate — le azioni online (pubblicare, contattare, chat) riprenderanno al ritorno della rete.", offlinePopMapBtn: "Vedi la mappa", offlinePopLater: "Più tardi", backOnline: "Riconnesso ✅",
    ...
    newJobToast: "🎉 Nuovo lavoro: {title} ({location}) è appena stato pubblicato!", smartJobToast: "🔔 Nuovo lavoro per te: {title} ({location})", invalidPhotoFormat: "Foto non valida: JPG/PNG/WEBP, max {mb} MB.", positionSavedComplete: "Posizione salvata! Completa il modulo.",
 ...
 newMsgs: "Nuovi messaggi",
    ...
    jobsNearby: "Lavori vicini", sortLabel: "Ordina:", sortNearest: "Più vicino", sortRating: "Migliori", sortPrice: "Prezzo più basso", sortNewest: "Più recente", sortProFirst: "⭐ Pro / verificati prima", emptyTitle: "Nessun lavoro trovato", emptySub: "Pubblica il primo lavoro in questa zona",
    ...
    adminDashboard: "Pannello Admin", secureChat: "Chat sicura", myListings: "I miei annunci", myProProfile: "Il mio profilo Pro", profileTypeLabel: "Tipo di profilo", profileTypePerson: "Persona", profileTypeCompany: "Azienda / Organizzazione", profileTypeCompanyHint: "Il nome dell'azienda sarà evidenziato con il badge 🏢.",
    ...
    verificationCodeExpired: "Codice scaduto (valido 24 h) — contattaci per una nuova verifica.",
  ...
  de: {
    qrPriceContext: "Was kostet es? (Anzeige: {price})",
    qrPriceFree: "Ist es kostenlos?",
    qrMissionThanks: "🙏 Danke für die Arbeit",
    qrRecommend: "⭐ Ich empfehle dich",
    specialtiesLabel: "Spezialgebiete",

    statRevenueLabel: "Geschätzte Einnahmen",
    statRevenueLocked: "Wechsle zu PRO, um deine geschätzten Einnahmen zu verfolgen.",
    statRevenueSub: "aus {n} besetzten Anzeigen",
    installSettingsBtn: "App installieren",
    installSettingsInstalled: "App installiert ✅",
    installManualTitle: "JobMarket auf deinem Handy installieren",
    installManualAndroid: "Android: Browsermenü ⋮ → „Zum Startbildschirm“ (oder „App installieren“)",
    installManualIos: "iPhone/iPad: Teilen-Button (Quadrat mit Pfeil nach oben) → „Zum Startbildschirm“",
    installManualNote: "Auf dem Startbildschirm aktualisiert sich die App bei jedem Öffnen — kein Store nötig.",
    okBtn: "OK",
    qualLabel: "Qualifikation (optional)",
    qualNone: "—",
    qualAuto: "Selbstgelernt / Erfahrung",
    qualCap: "Berufszeugnis / Praktische Ausbildung",
    qualBts: "Technisches Diplom (BTS) / Zertifiziert",
    qualIng: "Ingenieur / Zertifizierter Experte",
    completenessTitle: "Profilvollständigkeit",
    completenessMissing: "Zu ergänzen:",
    completenessComplete: "✅ Profil vollständig — bereit für mehr Kontakte!", autoReply: "Automatische Antwort", aiAwayFallback: "Hallo, hier ist {name} 👋 Deine Nachricht ist angekommen — ich antworte so schnell wie möglich.", portfolioTitle: "📸 Meine Arbeiten", portfolioSub: "Zeig deine besten Arbeiten (bis zu 6 Fotos). Kunden vertrauen einem Handwerker, der seine Arbeit zeigt.", portfolioPublicTitle: "📸 Seine Arbeiten", portfolioPhotosCount: "{n} Foto", aiSuggestBtn: "✨ KI-Antwort", guaranteeTitle: "🛡️ JobMarket-Garantie", guaranteeLine1: "Falls es Probleme mit diesem Anbieter gibt, schreitet unser Team ein und vermittelt.", guaranteeLine2: "Goldene Regel: Bezahle NIEMALS den vollen Betrag im Voraus — begleiche nach erledigter Arbeit.", aiSuggestLoading: "✨ Erstelle…", aiSuggestFallback: "Hallo, deine Nachricht ist angekommen. Ich bin diese Woche verfügbar und melde mich schnell bei dir.",
    availFilterEmptyTitle: "Niemand ist « jetzt verfügbar » hier",
    availFilterEmptySub: "Schalte den 🟢-Filter aus (auf « Dispo » tippen), um alle Anzeigen zu sehen.",
    availFilterOnToast: "🟢 Filter « Jetzt verfügbar » an: {n} Anzeige(n) von verfügbaren Personen.",
    availFilterEmptyToast: "Niemand ist « jetzt verfügbar » in dieser Zone. Tippe erneut auf 🟢, um alles zu sehen.",
    availFilterOffToast: "Filter « Jetzt verfügbar » aus: alle Anzeigen wieder sichtbar.",
    trustSignalsTitle: "🛡️ Vertrauenssignale",
    statRatingLabel: "Bewertungen",
    statMissionsLabel: "Aufträge",
    statConfirmedLabel: "Bestätigt",
    statReactivityLabel: "Reaktionszeit",
    statStatusLabel: "Status",
    ...
    offlinePopTitle: "Du bist offline", offlinePopBody: "Keine Internetverbindung. Die Karte bleibt für bereits geladene Bereiche nutzbar — Online-Aktionen (posten, kontaktieren, Chat) funktionieren wieder, wenn du wieder online bist.", offlinePopMapBtn: "Karte anzeigen", offlinePopLater: "Später", backOnline: "Wieder online ✅",
    ...
    newJobToast: "🎉 Neuer Auftrag: {title} ({location}) wurde gerade veröffentlicht!", smartJobToast: "🔔 Neuer Job für dich: {title} ({location})", invalidPhotoFormat: "Ungültiges Foto: JPG/PNG/WEBP, max. {mb} MB.", positionSavedComplete: "Position gespeichert! Füllen Sie das Formular aus.",
 ...
 newMsgs: "Neue Nachrichten",
    ...
    jobsNearby: "Jobs in der Nähe", sortLabel: "Sortieren:", sortNearest: "Nächste", sortRating: "Beste bewertet", sortPrice: "Günstigste", sortNewest: "Neueste", sortProFirst: "⭐ Pro / verifiziert zuerst", emptyTitle: "Keine Jobs gefunden", emptySub: "Veröffentlichen Sie den ersten Job in dieser Gegend",
    ...
    adminDashboard: "Admin-Bereich", secureChat: "Sicherer Chat", myListings: "Meine Anzeigen", myProProfile: "Mein Profi-Profil", profileTypeLabel: "Profilart", profileTypePerson: "Person", profileTypeCompany: "Firma / Organisation", profileTypeCompanyHint: "Der Firmenname wird mit dem 🏢-Badge hervorgehoben.",
    ...
    verificationCodeExpired: "Code abgelaufen (24 h gültig) — wende dich an uns für eine neue Verifizierung.",
  ...
  zh: {
    qrPriceContext: "多少钱？（发布：{price}）",
    qrPriceFree: "这是免费的吗？",
    qrMissionThanks: "🙏 感谢你的工作",
    qrRecommend: "⭐ 我推荐你",
    specialtiesLabel: "专长",

    statRevenueLabel: "预计收入",
    statRevenueLocked: "升级 PRO 以跟踪你的预计收入。",
    statRevenueSub: "来自 {n} 条已完成发布",
    installSettingsBtn: "安装应用",
    installSettingsInstalled: "应用已安装 ✅",
    installManualTitle: "将 JobMarket 安装到手机",
    installManualAndroid: "Android：浏览器菜单 ⋮ → 「添加到主屏幕」（或「安装应用」）",
    installManualIos: "iPhone/iPad：分享按钮（带向上箭头的方框）→ 「添加到主屏幕」",
    installManualNote: "放在主屏幕后，每次打开应用都会自动更新 — 无需应用商店。",
    okBtn: "好",
    qualLabel: "资质（可选）",
    qualNone: "—",
    qualAuto: "自学 / 经验丰富",
    ...
    qualBts: "技术文凭（BTS）/ 持证",
    qualIng: "工程师 / 认证专家",
    completenessTitle: "完整度",
    completenessMissing: "待完善：",
    completenessComplete: "✅ 资料已完善 — 准备好获得更多联系！", autoReply: "自动回复", aiAwayFallback: "您好，我是{name} 👋 您的消息已收到，我会尽快回复您。", portfolioTitle: "📸 我的作品", portfolioSub: "展示你最出色的作品（最多 6 张）。展示作品的工匠更值得客户信任。", portfolioPublicTitle: "📸 他的作品", portfolioPhotosCount: "{n} 张照片", aiSuggestBtn: "✨ AI 回复", guaranteeTitle: "🛡️ JobMarket 保障", guaranteeLine1: "如果与该服务商出现问题，我们的团队会介入调解。", guaranteeLine2: "黄金法则：切勿提前支付全部款项 — 工作完成后再付尾款。", aiSuggestLoading: "✨ 生成中…", aiSuggestFallback: "您好，您的消息已收到。我这周有空，会尽快给您详细答复。",
    availFilterEmptyTitle: "这里没有人「现在有空」",
    availFilterEmptySub: "关闭 🟢 筛选（点击「Dispo」按钮）查看全部发布。",
    availFilterOnToast: "🟢 「现在有空」筛选已开启：{n} 条可用人员的发布。",
    availFilterEmptyToast: "这个区域没有人「现在有空」。再次点击 🟢 按钮查看全部。",
    availFilterOffToast: "「现在有空」筛选已关闭：所有发布已恢复显示。",
    trustSignalsTitle: "🛡️ 信任信号",
    statRatingLabel: "评价",
    statMissionsLabel: "任务",
    statConfirmedLabel: "已确认",
    statReactivityLabel: "响应时间",
    statStatusLabel: "状态",
    ...
    offlinePopTitle: "您已离线", offlinePopBody: "当前没有网络连接。已加载的区域地图仍可浏览 — 发布、联系、聊天等联网操作将在网络恢复后继续。", offlinePopMapBtn: "查看地图", offlinePopLater: "稍后", backOnline: "已重新连接 ✅",
    ...
    newJobToast: "🎉 新工作:{title}({location})刚刚发布!", smartJobToast: "🔔 有一个适合你的新工作：{title}（{location}）", invalidPhotoFormat: "照片无效:仅支持JPG/PNG/WEBP,最大{mb} MB。", positionSavedComplete: "位置已保存!请填写表单。",
 ...
 newMsgs: "新消息",
    ...
    jobsNearby: "附近的工作", sortLabel: "排序", sortNearest: "最近", sortRating: "评分最高", sortPrice: "价格最低", sortNewest: "最新", sortProFirst: "⭐ Pro / 已认证优先", emptyTitle: "未找到工作", emptySub: "在该区域发布第一份工作",
    ...
    adminDashboard: "管理员面板", secureChat: "安全聊天", myListings: "我的发布", myProProfile: "我的专业资料", profileTypeLabel: "资料类型", profileTypePerson: "个人", profileTypeCompany: "企业 / 组织", profileTypeCompanyHint: "公司名称将带有🏢徽章突出显示。",
    ...
    verificationCodeExpired: "验证码已过期（24 小时内有效）— 请联系我们重新验证。",
...
function widgetI18nFor(lang) {
  ...
  return {
    ...
    star: L.ctxStar, unstar: L.ctxUnstar, pin: L.ctxPin, unpin: L.ctxUnpin, mute: L.muteLabel, unmute: L.unmuteLabel, archive: L.archiveLabel, unarchive: L.unarchiveLabel, del: L.ctxDelete, grpMsg: L.ctxGroupMsg, grpConv: L.ctxGroupConv, sel: L.ctxSelect, selCount: L.selCount, delMany: L.delManyConfirm, delManyAll: L.delManyAllConfirm,
    noStarred: L.noStarred, nStarred: L.nStarred, about: L.profileAbout, noMedia: L.noMedia, votes: L.votes, download: L.downloadBtn,
    lockNotice: L.lockNotice, newMsgs: L.newMsgs, autoReply: L.autoReply
  };
}
...
// URL du worker de notifications DIRECTES (voir jobmarket-direct-notify.js) :
// notifications ciblées en temps réel — « mission confirmée » (→ employeur)
// et « nouvel avis reçu » (→ artisan). À compléter APRÈS le déploiement du
// worker (voir LIRE_MOI.md) : url = la workers.dev du worker, secret = la
// variable d'env DIRECT_NOTIFY_SECRET (identique des deux côtés). Tant que
// url est vide, les appels sont simplement sautés (aucun impact).
const NOTIFY_DIRECT_CFG = {
  url: '',     // ex. 'https://jobmarket-direct-notify.ghislaintankat.workers.dev'
  secret: ''   // = DIRECT_NOTIFY_SECRET du worker
};
try { window.__directNotifyCfg = NOTIFY_DIRECT_CFG; } catch (e) {} // tests jsdom

// Appel générique au worker de notifs directes : silencieux, non bloquant,
// sauté si non configuré. Le worker vérifie identité + anti-usurpation +
// anti-spam côté serveur — l'app n'envoie que le minimum.
function notifyDirect(payload) {
  try {
    if (!NOTIFY_DIRECT_CFG.url) return;
    fetch(NOTIFY_DIRECT_CFG.url, {
      method: 'POST',
      headers: { 'X-Direct-Secret': NOTIFY_DIRECT_CFG.secret, 'Content-Type': 'application/json' },
      body: JSON.stringify(payload)
    }).catch(() => {});
  } catch (e) { console.warn('notifyDirect error', e); }
}
...
function updateAccountUI(user) {
  ...
  db.ref('profiles/' + user.uid).once('value').then(snapshot => {
    ...
    if (emailEl) emailEl.textContent = user.email || (user.phoneNumber ? '📱 ' + user.phoneNumber : 'Connexion Google');
...
// Photos du chat : version « largeur limitée, SANS recadrage » pour la bulle
// (vignette ~220 px → 640 px couvre les écrans jusqu'à x3), + q_auto,f_auto
// (qualité/format optimaux par appareil). Ça économise des Mo de 4G : avant,
// on téléchargeait la photo PLEINE RÉSOLUTION pour l'afficher à 220 px.
// La pleine taille reste réservée à la lightbox (tap). Les URLs qui ne sont
// pas Cloudinary (data: pendant l'envoi optimiste, autres hébergements)
  ...
  return `
      <div style="min-width:240px;font-family:'DM Sans',sans-serif;">
        ...
        ${(() => {
          ...
          if (p.profileType === 'company' && p.company) {
            return `<div style="display:inline-flex;align-items:center;gap:5px;font-size:12px;font-weight:600;color:var(--blue);margin-bottom:6px;"><span title="${t('profileTypeCompany')}">🏢</span>${escapeHtml(p.company)}</div>`;
          }
          return '';
        })()}
        ...
        ${(() => {
          const p = profilesCache[job.user] || {};
          let badge = '';
          // Badge PRO doré (abonné actif) — en tête, comme sur les cartes
...
function syncJobs() {
  try {
    ...
    jobsQuery.on('child_changed', snap => {
      ...
      applyFilters();
      recalcJobStats();
      // Confirmation de mission en temps réel : si c'est le job lié au chat
      // ouvert (ex : status → « pourvu »), le bouton apparaît immédiatement.
      if (typeof refreshJobBarConfirmForOpenChat === 'function') refreshJobBarConfirmForOpenChat(jobId);
    });
...
// ===== LISTE DES JOBS — RENDU PAR LOTS (scroll infini) =====
// Au lieu de rendre TOUTES les cartes d'un coup (lourd avec beaucoup
// d'annonces : DOM géant, animations déclenchées en masse, scroll
// saccadé sur mobile bas de gamme), on affiche JOBS_LIST_PAGE_SIZE cartes,
// puis on ajoute le lot suivant quand on approche du bas du panneau
// (seuil 400 px, listener passif pour ne jamais bloquer le scroll).
const JOBS_LIST_PAGE_SIZE = 60;
let jobsListPending = [];      // la liste complète (filtrée + triée) à afficher
let jobsListRenderedCount = 0; // nombre de cartes déjà rendues
let jobsListSignature = '';    // empreinte (ids triés) du dernier contenu rendu
let jobsRefreshTimer = null;   // debounce des rafraîchissements déclenchés par les profils

// Rend des lots jusqu'à atteindre `target` cartes (utilisé pour RESTAURER la
// profondeur de scroll après un re-rendu du même contenu, voir updateJobsList).
function renderJobsUpTo(target) {
  while (jobsListRenderedCount < target && jobsListRenderedCount < jobsListPending.length) {
    renderNextJobsListBatch();
  }
}
// Rafraîchissement DIFFÉRÉ de la liste + des popups carte : les mises à jour
// de profil arrivent souvent en rafale (ratingAvg/ratingCount recalculés après
// chaque avis par rating-sync.js). Sans debounce, CHAQUE tick relançait un
// re-rendu complet de la liste (et de tous les popups) — coûteux et, avec le
// rendu par lots, il remontrait l'utilisateur en haut de liste. On coalesce
// les ticks en un seul re-rendu ~150 ms plus tard (imperceptible à l'œil).
  ...
  return `
    <div class="job-card" style="animation-delay:${i*0.04}s${(job.boosted && job.boostedUntil > Date.now()) ? ';border:1px solid var(--gold,#FFD700);' : ''}${job.status === 'filled' ? ';opacity:0.6;' : ''}" onclick="focusJob(${job.lat},${job.lng}); openJobPreview('${job.id}')">
      ...
      <div class="job-card-body">
        <div class="job-card-title">${job.status === 'filled' ? '✅ ' : ''}${(job.boosted && job.boostedUntil > Date.now()) ? '🚀 ' : ''}${((profilesCache[job.user]||{}).isPro && ((profilesCache[job.user]||{}).proUntil||0) > Date.now()) ? '⭐ ' : ''}${savedJobIds.has(job.id) ? '🔖 ' : ''}${((profilesCache[job.user]||{}).qualification === 'cap' ? '<span title="'+t('qualCap')+'" style="font-size:10px;font-weight:700;color:var(--blue,#2D6CDF);">🎓CAP</span> ' : '') + ((profilesCache[job.user]||{}).qualification === 'bts' ? '<span title="'+t('qualBts')+'" style="font-size:10px;font-weight:700;color:var(--blue,#2D6CDF);">🎓BTS</span> ' : '') + ((profilesCache[job.user]||{}).qualification === 'ing' ? '<span title="'+t('qualIng')+'" style="font-size:10px;font-weight:700;color:var(--blue,#2D6CDF);">🎓Ing.</span> ' : '')}${escapeHtml(job.title)}</div>
      ...
      <div class="job-card-right">
        ...
        ${(() => {
          ...
          // Badge ENTREPRISE (profil entreprise) — l'identité de l'entreprise se démarque
          if (p.profileType === 'company') {
            badge += `<span title="${t('profileTypeCompany')}" style="font-size:11px;">🏢</span>`;
          }
          if (p.verified) {
          ...
          const rating = p.ratingCount ? `<span style="font-size:11px;color:#FFD700;font-weight:700;">★ ${(p.ratingAvg || 0).toFixed(1)}</span>` : '';
...
function renderNextJobsListBatch() {
  ...
  if (!list) return;
  const slice = jobsListPending.slice(jobsListRenderedCount, jobsListRenderedCount + JOBS_LIST_PAGE_SIZE);
  list.insertAdjacentHTML('beforeend', slice.map((job, i) => renderJobCardHtml(job, jobsListRenderedCount + i)).join(''));
  jobsListRenderedCount += slice.length;
}
function bindListInfiniteScroll(el) {
  if (el._infiniteScrollBound) return; // un seul listener par liste
  el._infiniteScrollBound = true;
  const scroller = el.closest('.sheet') || el.parentElement; // c'est le panneau qui scroll
  if (!scroller) return;
  scroller.addEventListener('scroll', () => {
    if (jobsListRenderedCount >= jobsListPending.length) return;
    if (scroller.scrollTop + scroller.clientHeight >= scroller.scrollHeight - 400) renderNextJobsListBatch();
  }, { passive: true });
}
function updateJobsList(jobs) {
  if (currentAvailFilter) jobs = jobs.filter(j => isProviderAvailable(j.user)); // filtre « 🟢 Dispo »
  jobs = sortJobsForList(jobs); // tri « Jobs proches » (critère persisté par l'utilisateur)
  const list = document.getElementById('listContent');
  if (!list) return;
  ...
  // Mémo de perf + correction de UX : si le contenu rendu est IDENTIQUE au
  // précédent (même ensemble d'ids, même ordre — ex : mise à jour d'un badge
  // ou d'une note sans ajout/retrait de job), on ne remonte PAS à zéro : on
  // restaure la même profondeur de cartes, donc la position de scroll est
  // conservée (sinon un simple refresh de profil remonterait l'utilisateur
  // en haut de liste quand il était scrollé profondément). Si le contenu a
  // changé (filtre, tri, ajout/suppression de job), on repart du 1er lot.
  const sig = jobs.map(j => j.id).join('|');
  const sameContent = sig === jobsListSignature && jobs.length > 0;
  jobsListSignature = sig;
  if (!sameContent) jobsListRenderedCount = 0;

  if (jobs.length === 0) {
    // Filtre « Dispo » actif → message dédié (sinon « Publiez le premier job »
    // qui n'a rien à voir avec la cause réelle).
    const availOn = currentAvailFilter;
    list.innerHTML = `<div class="empty-state">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">${availOn ? '<circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 3"/>' : '<circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/>'}</svg>
      <div class="empty-title">${availOn ? t('availFilterEmptyTitle') : '<span data-i18n="emptyStateNoJobs">Aucun job trouvé</span>'}</div>
      <div class="empty-sub">${availOn ? t('availFilterEmptySub') : '<span data-i18n="emptyZoneSub">Publiez le premier job dans cette zone</span>'}</div>
    </div>`;
    return;
  }

  list.innerHTML = '';
  renderJobsUpTo(sameContent ? Math.max(jobsListRenderedCount, JOBS_LIST_PAGE_SIZE) : JOBS_LIST_PAGE_SIZE);
  bindListInfiniteScroll(list);    // lots suivants au scroll
}
...
// ===== ROUTE =====
// leaflet-routing-machine (~40 Ko de JS) n'est plus chargé à l'ouverture de
// la page : il n'est nécessaire QUE quand l'utilisateur demande un itinéraire.
// Chargement à la demande, une seule fois (promise mise en cache), avec repli
// propre si le CDN échoue (toast, sans casser le reste de la page).
let routingMachineLoadPromise = null;
function ensureRoutingMachineLoaded() {
  if (window.L && window.L.Routing) return Promise.resolve();
  if (!routingMachineLoadPromise) {
    routingMachineLoadPromise = new Promise((resolve, reject) => {
      ...
      s.onload = () => resolve();
      s.onerror = () => { routingMachineLoadPromise = null; reject(new Error('leaflet-routing-machine failed to load')); };
...
async function drawRoute(lat, lng) {
  ...
  try { await ensureRoutingMachineLoaded(); }
  catch (e) { console.error(e); showToast(t('toastGenericError'), 'error'); return; }
...
// N'affiche que les annonces d'artisans qui se sont marqués « disponible
// maintenant » (le statut expire après 24 h — même règle que le badge des
// cartes, cf. availabilityBadge). Persisté par l'utilisateur. S'applique à
// la liste du panneau (tous les points d'entrée, via updateJobsList) ET aux
// marqueurs de la carte (via applyFilters).
let currentAvailFilter = false;
try { currentAvailFilter = localStorage.getItem('availFilter') === '1'; } catch (e) {}
function isProviderAvailable(uid) {
  const p = profilesCache[uid] || {};
  if (p.availability !== 'now') return false;
  return Date.now() - (p.availabilityUpdated || 0) <= 24 * 60 * 60 * 1000;
}
function updateAvailFilterBtn() {
  const b = document.getElementById('availFilterBtn');
  if (b) b.classList.toggle('active', currentAvailFilter);
}
function toggleAvailFilter() {
  currentAvailFilter = !currentAvailFilter;
  try { localStorage.setItem('availFilter', currentAvailFilter ? '1' : '0'); } catch (e) {}
  updateAvailFilterBtn();
  try { const _jobSortSel = document.getElementById('jobSortSel'); if (_jobSortSel) _jobSortSel.value = currentJobSort; } catch (e) {}
  // Onglet « Sauvegardés » : ne ré-affiche que sa liste (l'applyFilters
  // ré-afficherait la liste « Tous »). Le filtre est appliqué de toute
  // façon dans updateJobsList.
  let n = null;
  if (jobsSheetFilter === 'saved') showSavedJobsInSheet(); else n = applyFilters();
  // ⚠️ Confusion historique : ce bouton vert filtre la liste (il n'active
  // PAS la dispo de l'utilisateur — c'est le bouton de son profil qui le
  // fait). Sans explication, l'activation vidait la liste (« les jobs
  // disparaissent »). Maintenant le toast + l'état vide le disent clair.
  if (currentAvailFilter) {
    if (n === 0) showToast(t('availFilterEmptyToast'), 'error');
    else if (n != null) showToast(t('availFilterOnToast').replace('{n}', n), 'info');
  } else {
    showToast(t('availFilterOffToast'), 'info');
  }
}

// ===== TRI DE LA LISTE « JOBS PROCHES » =====
// 5 critères : le plus proche (défaut) / ⭐ Pro-vérifié d'abord / mieux notés
// / moins cher / plus récent. Le choix est persisté (localStorage). Le tri
// s'applique dans updateJobsList (le seul point d'affichage) → les onglets
// « Tous » et « Sauvegardés » sont tous les deux triés, sans toucher aux
// marqueurs de la carte (l'ordre n'a pas de sens pour eux).
let currentJobSort = 'dist';
try { const _jobSortSaved = localStorage.getItem('jobSort'); if (['dist', 'pro', 'rating', 'price', 'newest'].includes(_jobSortSaved)) currentJobSort = _jobSortSaved; } catch (e) {}
function parseJobPrice(job) {
  try { if (typeof isFreePrice === 'function' && isFreePrice(job)) return 0; } catch (e) {}
  const n = parseFloat(String(job.price || '').replace(/[\s\u00A0\u202F,]/g, ''));
  return isNaN(n) ? Infinity : n;
}
// ===== ⭐ PRO / VÉRIFIÉ D'ABORD (roadmap #1 — « toujours en haut ») =====
// Échelle de confiance : 0 = Pro actif (abonnement non expiré),
// 1 = identité vérifiée, 2 = le reste. Le tri « pro » classe par échelle,
// puis par distance DANS chaque groupe (règle d'or : trouver un bon
// artisan RAPIDEMENT — le plus proche des bons d'abord).
function proTrustTier(job) {
  const p = profilesCache[job.user] || {};
  if (p.isPro && (p.proUntil || 0) > Date.now()) return 0;
  if (p.verified) return 1;
  return 2;
}
function sortJobsForList(jobs) {
  const arr = [...jobs];
  const dist = j => (j.dist >= 999 ? 1e9 : (j.dist || 0)); // position inconnue → tout à la fin
  const rating = j => { const p = profilesCache[j.user] || {}; return p.ratingCount ? (p.ratingAvg || 0) : -1; };
  switch (currentJobSort) {
    case 'pro': arr.sort((a, b) => proTrustTier(a) - proTrustTier(b) || dist(a) - dist(b)); break;
    case 'rating': arr.sort((a, b) => rating(b) - rating(a) || dist(a) - dist(b)); break;
    case 'price': arr.sort((a, b) => parseJobPrice(a) - parseJobPrice(b) || dist(a) - dist(b)); break;
    case 'newest': arr.sort((a, b) => (b.timestamp || 0) - (a.timestamp || 0)); break;
    default: arr.sort((a, b) => dist(a) - dist(b));
  }
  return arr;
}
function setJobSort(mode) {
  if (!['dist', 'pro', 'rating', 'price', 'newest'].includes(mode)) mode = 'dist';
  currentJobSort = mode;
  try { localStorage.setItem('jobSort', mode); } catch (e) {}
  const sel = document.getElementById('jobSortSel');
  if (sel) sel.value = mode;
  // Ré-affiche l'onglet ouvert (le tri vit dans updateJobsList)
  if (jobsSheetFilter === 'saved') showSavedJobsInSheet(); else showAllJobsInSheet();
}
...
function applyFilters() {
  ...
  const filtered = allJobs.filter(job => {
    ...
    const matchesAvail = !currentAvailFilter || isProviderAvailable(job.user);
    return matchesCategory && matchesSearch && matchesRadius && notFilled && matchesAvail;
  });
  ...
  return filtered.length; // permet au toggle du filtre « Dispo » d'expliquer le résultat
}
...
// Approuver = les documents ont été vérifiés par un HUMAIN → le badge
// "verified" est posé ICI, par l'admin (champs verified/verifiedAt réservés
// aux admins dans database.rules.json — le client ne les écrit plus jamais).
// On génère aussi un code à 7 chiffres (valable 24 h), enregistré sur le
// profil, et on ouvre WhatsApp pré-rempli vers le numéro du demandeur pour le
// lui transmettre : la saisie du code dans l'app (confirmVerificationCode)
// prouve la POSSESSION du numéro → phoneVerified uniquement. Les documents
// restent visibles tant que le code n'a pas été confirmé.
async function approveVerification(uid) {
  ...
  try {
    ...
    await db.ref('profiles/' + uid).update({ verificationCode: code, verificationCodeAt: Date.now(), verified: true, verifiedAt: Date.now(), verificationRequested: null });
...
// ===== BOUTON « INSTALLER L'APP » DANS LES PARAMÈTRES =====
// Permet de (ré)installer l'app sur l'écran d'accueil à tout moment
// (notamment après l'avoir désinstallée) : si le navigateur propose le
// prompt natif (Android/Chrome, Edge…), on l'ouvre directement ; sinon
// (iOS Safari, etc.) on affiche les instructions manuelles.
function isStandaloneApp() {
  try { return window.matchMedia('(display-mode: standalone)').matches || window.navigator.standalone === true; } catch (e) { return false; }
}
function refreshInstallAppBtn() {
  const btn = document.getElementById('installAppBtnLabel');
  if (btn) btn.textContent = isStandaloneApp() ? t('installSettingsInstalled') : t('installSettingsBtn');
}
async function installAppFromSettings() {
  if (isStandaloneApp()) { showToast(t('installSettingsInstalled'), 'info'); return; }
  if (deferredInstallPrompt) {
    deferredInstallPrompt.prompt();
    try { await deferredInstallPrompt.userChoice; } catch (e) { /* ignoré */ }
    deferredInstallPrompt = null; // prompt consommé (accepté ou fermé)
    refreshInstallAppBtn();
    return;
  }
  // Pas de prompt natif (iOS Safari ne déclenche jamais beforeinstallprompt)
  const ov = document.getElementById('installManualOverlay');
  if (ov) {
    ov.querySelectorAll('[data-i18n]').forEach(el => { try { el.textContent = t(el.getAttribute('data-i18n')); } catch (e) {} });
...
function writePresenceState() {
  ...
  // .catch obligatoire : en cas de permission_denied (ex. déconnexion en
  // cours), un set refusé sans catch devenait une rejection non gérée →
  // un « Uncaught (in promise) PERMISSION_DENIED » dans la console à
  // chaque changement d'onglet (c'était la source de la boucle de 99+
  // erreurs). L'état de présence est une info secondaire : un échec est
  // sans conséquence, il ne doit JAMAIS polluer la console.
...
function teardownPresence() {
  if (presenceUid) {
    // Même .catch : la déconnexion se fait APRES le changement de session,
    // auth.uid n'est plus celui de presenceUid → permission_denied garanti
    // sans catch (c'était l'autre source de la boucle d'erreurs).
    db.ref('presence/' + presenceUid).set({
      state: 'inactive',
      lastChanged: firebase.database.ServerValue.TIMESTAMP
    }).catch(() => {});
...
// ===== ALERTES INTELLIGENTES (roadmap #7) =====
// Avant : UN toast pour CHAQUE job publié au Cameroun — même pour celui qui
// vient de publier, même pour une catégorie/quartier qui ne l'intéresse pas.
// Maintenant : on n'alerte QUE si le job concerne l'artisan, avec les même
// règles que le push serveur (scripts/sendNotifications.js) :
//   - CATÉGORIE : activée sauf si décochée dans les préférences
//     (notifyPrefs/{uid}/{cat} === false — modèle sparse, absence = activé) ;
//   - DISTANCE : dans le rayon maxDistanceKm (défaut 25 km) si la position
//     est connue (GPS ou dernière position du profil) — sans position, pas
//     de filtre de distance (impossible de filtrer) ;
//   - JAMAIS au publicateur du job (job.user === moi).
// Non connecté : toast générique (l'app "vit", pas de profil pour filtrer).
let chargementInitialJobs = true;

// Préférences en cache mémoire 30 s : le toast d'un nouveau job ne doit pas
// attendre un aller-retour réseau (ni en rajouter à chaque job).
let notifPrefsCache = null, notifPrefsCacheUid = null, notifPrefsCacheTs = 0;
async function fetchNotifPrefs(uid) {
  if (notifPrefsCacheUid === uid && notifPrefsCacheTs && Date.now() - notifPrefsCacheTs < 30000) return notifPrefsCache;
  try {
...
// Position pour le filtre de distance : GPS live, sinon la dernière position
// connue du profil (écrite par locateMe() quand l'utilisateur s'est localisé).
function myLocationForAlerts(uid) {
  if (userCoords && typeof userCoords.lat === 'number' && typeof userCoords.lng === 'number') return userCoords;
  const p = profilesCache[uid] || {};
  if (typeof p.lat === 'number' && typeof p.lng === 'number') return { lat: p.lat, lng: p.lng };
  return null;
}

// 'generic' = toast classique (pas de filtre possible), 'smart' = job qui
// concerne l'artisan, 'none' = pas d'alerte. Fonction PURE : testable telle
// quelle (c'est elle qui porte toute la logique de matching).
function classifyJobAlert(job, me, prefs, myLoc) {
  if (!me || me.isAnonymous) return 'generic';
  if (job.user === me.uid) return 'none'; // jamais au publicateur
  prefs = prefs || {};
  if (prefs[job.icon] === false) return 'none'; // catégorie décochée
  const maxKm = (typeof prefs.maxDistanceKm === 'number') ? prefs.maxDistanceKm : 25;
  if (myLoc && job.lat != null && job.lng != null) {
    if (calcDist(myLoc.lat, myLoc.lng, job.lat, job.lng) > maxKm) return 'none';
  }
  return 'smart';
}
...
async function smartJobAlert(job) {
  ...
  try {
    if (me && !me.isAnonymous) {
      ...
      const myLoc = myLocationForAlerts(me.uid);
      cls = classifyJobAlert(job, me, prefs, myLoc);
      if (cls === 'smart' && myLoc && job.lat != null && job.lng != null) {
        const d = calcDist(myLoc.lat, myLoc.lng, job.lat, job.lng);
        if (d < 999) { job.distForAlert = d; }
      }
    }
  } catch (e) { cls = 'generic'; }
  if (cls === 'none') return;
  let msg;
  if (cls === 'generic') {
    msg = t('newJobToast').replace('{title}', job.title || '').replace('{location}', job.landmark || 'Cameroun');
    showToast(msg, 'info');
  } else {
    msg = t('smartJobToast').replace('{title}', job.title || '').replace('{location}', job.landmark || (job.icon ? getCatLabel(job.icon) : '') || '');
    if (job.distForAlert != null) msg += ' · ' + job.distForAlert.toFixed(1) + ' km';
    showToast(msg, 'success');
    vibrateDevice(80); // retour haptique (Android) — l'alerte "sonne" aussi
  }
}
...
db.ref('jobs').limitToLast(1).on('child_added', (snapshot) => {
  ...
  smartJobAlert(nouveauJob); // match catégorie/distance/publicateur (roadmap #7)
  refreshNotifPanel();
});
...
async function viewUserProfile(uid) {
    ...
    try {
        ...
        // Qualification (optionnel) : lisible mais non modifiable sur un profil public
        const qualSel = document.getElementById('profileQualification');
        if (qualSel) { qualSel.value = data.qualification || ''; qualSel.disabled = true; }
        const qualTag = document.getElementById('profileQualTag');
        if (qualTag) {
            if (data.qualification && qualSel.value) {
                qualTag.style.display = 'block';
                qualTag.textContent = '🎓 ' + t('qual' + qualSel.value.charAt(0).toUpperCase() + qualSel.value.slice(1));
            } else qualTag.style.display = 'none';
        }
        setProfileType(data.profileType === 'company' ? 'company' : 'person');
        ...
        if (headerTitle) headerTitle.innerHTML = t("artisanProfile") + profileTypeTag(data);
        ...
        // Le bloc « Mes badges » (profil privé) ne doit JAMAIS filtrer sur un
        // profil public : on le masque et on affiche les badges de confiance.
        const achBox = document.getElementById('achievementsBox');
        if (achBox) achBox.style.display = 'none';
        // Idem pour le tableau de bord perso (statistiques + signaux de
        // confiance) : données privées, jamais affichées sur un profil public.
        const proStatsBox = document.getElementById('proStatsBox');
        if (proStatsBox) proStatsBox.style.display = 'none';
        // Le meter de complétion est un outil PRIVÉ (motivation à compléter) :
        // jamais affiché sur un profil public.
        const completenessBox = document.getElementById('profileCompletenessBox');
        if (completenessBox) completenessBox.style.display = 'none';
        // Portfolio : affiché en lecture seule (photos de l'autre) — avant,
        // le contenu du profil ouvert juste avant (ex. le sien, avec les
        // boutons de suppression) filtrait sur le profil public.
        renderPublicPortfolio(data);
        // Plus visible pour le visiteur : le portfolio passe juste après les
        // badges/missions (sa position d'origine, après les avis, était basse)
        const portBox2 = document.getElementById('portfolioBox');
        const verifBox2 = document.getElementById('verificationStatusBox');
        if (portBox2 && verifBox2) verifBox2.parentNode.insertBefore(portBox2, verifBox2);

        renderProfileBadgeAndRating(data);
        renderPublicTrustBadges(data);
        renderPublicMissions(data);
        // Garantie JobMarket (roadmap #8) : message de confiance AUX CLIENTS,
        // affiché uniquement ici (profil public), jamais sur le profil privé.
        const guarBox2 = document.getElementById('guaranteeBox');
        if (guarBox2) guarBox2.style.display = 'block';
        renderOwnerReviews(uid, 'profileReviewsBox');
    } catch (error) {
...
async function openProfileSheet() {
    ...
    try {
        ...
        // Qualification (optionnel) : éditable sur mon propre profil
        const qualSel = document.getElementById('profileQualification');
        if (qualSel) { qualSel.value = data.qualification || ''; qualSel.disabled = false; }
        const qualTag = document.getElementById('profileQualTag');
        if (qualTag) qualTag.style.display = 'none'; // le select suffit ici (mode édition)
        setProfileType(data.profileType === 'company' ? 'company' : 'person');
        ...
        const headerTitle = document.querySelector('.profile-sheet-header h2');
        if (headerTitle) headerTitle.innerHTML = t("myProProfile") + profileTypeTag(data);
        ...
        // Retour au profil PRIVÉ : « Mes badges » visible, badges publics masqués
        // (évite l'inverse d'un profil public consulté juste avant).
        const achBox = document.getElementById('achievementsBox');
        ...
        if (guarBox) guarBox.style.display = 'none';
        const proStatsBox = document.getElementById('proStatsBox');
        if (proStatsBox) proStatsBox.style.display = 'block';
        // Retour au profil privé : portfolio éditable (bouton d'ajout visible)
        const portBox = document.getElementById('portfolioBox');
        if (portBox) portBox.style.display = 'block';
        const portAddBtn = document.getElementById('portfolioAddBtn');
        if (portAddBtn) portAddBtn.style.display = '';
        // Labels « à moi » + position d'origine (après les badges) — le flow
        // public les déplace/remplace
        const pt2 = document.getElementById('portfolioTitle');
        const ps2 = document.getElementById('portfolioSub');
        if (pt2) pt2.textContent = t('portfolioTitle');
        if (ps2) ps2.textContent = t('portfolioSub');
        const achBox2 = document.getElementById('achievementsBox');
        if (portBox && achBox2) achBox2.after(portBox);
        ...
        profileLiveRef.on('value', (snap) => {
            ...
            renderProfileCompleteness(freshData);
        });
    } catch (error) {
        console.error("Erreur profil:", error);
    }
...
// Portfolio AFFICHÉ EN LECTURE SEULE sur un profil PUBLIC : photos de
// l'autre (pas de boutons de suppression — ce serait bizarre et l'écriture
// serait refusée de toute façon), et surtout PAS le contenu d'un profil
// précédemment ouvert (ex. le sien) qui ne doit jamais filtrer.
function renderPublicPortfolio(profile) {
    ...
    const photos = (profile && profile.portfolio) ? Object.values(profile.portfolio) : [];
    if (!photos.length) { box.style.display = 'none'; return; }
    box.style.display = 'block';
    // Titre/sous-titre « visiteur » (le flow privé remet « mes réalisations »)
    const title = document.getElementById('portfolioTitle');
    const sub = document.getElementById('portfolioSub');
    if (title) title.textContent = t('portfolioPublicTitle');
    if (sub) sub.textContent = t('portfolioPhotosCount').replace('{n}', photos.length);
    const gal = document.getElementById('portfolioGallery');
    if (gal) {
        // Vignettes cliquables : le visiteur voit chaque réalisation en grand
        gal.innerHTML = photos.map(url => `
        <div onclick="openImageLightbox('${url}')" style="width:110px;height:110px;border-radius:10px;overflow:hidden;border:1px solid var(--border,#333);cursor:zoom-in;">
          <img src="${escapeHtml(url)}" style="width:100%;height:100%;object-fit:cover;" loading="lazy" alt="${t('altProfilePhotoPreview')}">
        </div>`).join('');
    }
}
...
async function confirmVerificationCode() {
    ...
    try {
        ...
        // Code expiré (24 h) : les règles l'appliquent aussi (l'écriture serait
        // refusée), mais ici le message est clair au lieu d'une erreur réseau.
        const codeAt = Number(profile.verificationCodeAt) || 0;
        if (codeAt && Date.now() - codeAt > 24 * 3600 * 1000) {
        ...
        await db.ref('profiles/' + user.uid).update({
            // verified/verifiedAt ne sont PLUS écrits par le client : l'admin
            // les pose à l'approbation (champs réservés aux admins dans
            // database.rules.json). Le code prouve la possession du numéro
            // WhatsApp fourni → phoneVerified (seul champ "badge" écrit ici).
            phoneVerified: true,
...
// ==========================================
// TYPE DE PROFIL : personne (défaut) ou entreprise/organisation
// ==========================================
// Un profil « entreprise » s'affiche avec un 🏢 (profil, fiche publique,
// panneau contact du chat) : le champ « Société » devient le nom principal
// de l'activité. Rien d'autre ne change (règles Firebase : le champ est
// libre, aucune modification de règles nécessaire).
let currentProfileType = 'person';
function setProfileType(type) {
  currentProfileType = (type === 'company') ? 'company' : 'person';
  renderProfileTypeButtons();
}
function renderProfileTypeButtons() {
  const p = document.getElementById('profileTypePersonBtn');
  const c = document.getElementById('profileTypeCompanyBtn');
  if (p) p.classList.toggle('active', currentProfileType !== 'company');
  if (c) c.classList.toggle('active', currentProfileType === 'company');
  const hint = document.getElementById('profileTypeHint');
  if (hint) hint.textContent = currentProfileType === 'company' ? t('profileTypeCompanyHint') : '';
}
// Petit tag 🏢 pour les affichages (profil, fiche publique, contact du chat)
function profileTypeTag(profile) {
  return (profile && profile.profileType === 'company') ? ' 🏢' : '';
}
// Nom d'affichage principal d'un profil : pour un profil ENTREPRISE, le nom de
// la société est mis en avant (s'il est renseigné), sinon le nom, puis la
// société. Rend le profil entreprise une identité de premier plan.
function profileDisplayName(profile) {
  if (!profile) return '';
  if (profile.profileType === 'company' && profile.company) return profile.company;
  return profile.name || profile.company || '';
}
// Badge 🏢 compact pour les profils entreprise (carte job, popup, contact).
function profileTypeBadge(profile) {
  if (profile && profile.profileType === 'company') {
    return `<span title="${t('profileTypeCompany')}" style="font-size:11px;" aria-label="${t('profileTypeCompany')}">🏢</span>`;
  }
  return '';
}
...
async function saveProfile() {
    ...
    try {
        const updateData = {
            ...
            jobTitle,
            skills,
            profileType: currentProfileType, // 'person' (défaut) | 'company'
            qualification: (document.getElementById('profileQualification') || {}).value || null, // optionnel
...
// ==========================================
// Lightbox générique : ouvre une photo en PLEIN ÉCRAN (portfolio du profil —
// le chat a sa propre lightbox waLB). Un tap n'importe où referme.
function openImageLightbox(url) {
    ...
    if (!ov) {
        ...
        ov.id = 'imgLightbox';
        ov.style.cssText = 'position:fixed;inset:0;z-index:2147483646;background:rgba(0,0,0,0.92);display:none;align-items:center;justify-content:center;padding:16px;cursor:zoom-out;';
        ov.innerHTML = '<img id="imgLightboxImg" style="max-width:100%;max-height:100%;border-radius:8px;object-fit:contain;" alt="">';
...
function renderPortfolio(profile) {
    ...
    gallery.innerHTML = photos.map(([key, url]) => `
        <div style="position:relative;width:110px;height:110px;border-radius:10px;overflow:hidden;border:1px solid var(--border,#333);">
            <img src="${url}" style="width:100%;height:100%;object-fit:cover;cursor:zoom-in;" loading="lazy" onclick="openImageLightbox('${url}')">
...
// Structure : profileViews/{uid}/views/{semaine} et profileViews/{uid}/viewsTotal
// (nœud dédié en haut niveau — le .write de profiles/{uid} étant réservé à
// l'owner/aux admins, un visiteur ne pouvait pas écrire dans le profil : le
// compteur vivait donc hors du profil, avec la règle « incrément strict +1 »)
function countProfileView(ownerUid) {
    const week = getWeekKey();
    const base = db.ref('profileViews/' + ownerUid);
    // ServerValue.increment plutôt que transaction : une transaction exige le
    // droit de LIRE le nœud (réservé à l'owner/aux admins), or le compteur
    // est incrémenté par n'importe quel VISITEUR — même motif que
    // bumpDailyStat(). L'incrément serveur est atomique et sans lecture.
    base.child('views/' + week).set(firebase.database.ServerValue.increment(1)).catch(() => {});
    base.child('viewsTotal').set(firebase.database.ServerValue.increment(1)).catch(() => {});
}
...
// 💰 REVENUS ESTIMÉS (dashboard style Wassa, renforcé) : total des PRIX des
// annonces pourvues par l'utilisateur. Données déjà en mémoire (allJobs,
// temps réel) → ZÉRO lecture réseau en plus. « Gratuit » = 0, prix illisible
// (Infinity) ignoré. C'est une ESTIME (le vrai montant se négocie en cash) —
// l'étiquette le dit pour rester honnête.
function computeMyRevenue(uid) {
  let total = 0, count = 0;
  try {
    for (const j of allJobs) {
      if (!j || j.user !== uid || j.status !== 'filled') continue;
      const p = parseJobPrice(j);
      if (!isFinite(p) || p <= 0) continue; // gratuit ou prix illisible
      total += p; count++;
    }
  } catch (e) {}
...
async function renderProStats(profile) {
    ...
    if (!box) return;
    // Les vues vivent dans le nœud dédié profileViews/{uid} (les visiteurs
    // n'écrivant pas dans le profil — voir countProfileView) : 1 lecture.
    const uid = (auth.currentUser && auth.currentUser.uid) || (profile && profile.uid);
    let viewsNode = {};
    try { const s = await db.ref('profileViews/' + uid).once('value'); viewsNode = s.val() || {}; } catch (e) {}
    const stats = { views: viewsNode.views || {}, viewsTotal: viewsNode.viewsTotal || 0 };
    const week = getWeekKey();
    ...
    const rev = uid ? computeMyRevenue(uid) : { total: 0, count: 0 };
    const revTotalHtml = rev.total > 0 ? rev.total.toLocaleString('fr-FR') + ' <span style="font-size:12px;font-weight:700;">XAF</span>' : '0 <span style="font-size:12px;font-weight:700;">XAF</span>';
    ...
    if (isPro) {
        html = `
          ...
          <div style="margin-top:10px;text-align:center;padding:14px 12px;background:rgba(37,211,102,0.10);border:1px solid rgba(37,211,102,0.35);border-radius:10px;">
            <div style="font-size:12px;font-weight:700;color:#25D366;margin-bottom:4px;">💰 ${t('statRevenueLabel')}</div>
            <div style="font-size:26px;font-weight:800;color:#25D366;">${revTotalHtml}</div>
            <div style="font-size:11px;color:var(--text-dim,#999);margin-top:4px;">${rev.count ? t('statRevenueSub').replace('{n}', rev.count) : t('statRevenueSub').replace('{n}', '0')}</div>
          </div>`;
    } else {
        html = `
          ...
          <div style="margin-top:10px;padding:14px 12px;background:rgba(255,255,255,0.04);border-radius:10px;text-align:center;">
            <div style="filter:blur(4px);user-select:none;font-size:22px;font-weight:800;color:#25D366;">••• XAF</div>
            <div style="font-size:12px;color:var(--text-dim,#999);margin-top:8px;">🔒 ${t('statRevenueLocked')}</div>
          </div>`;
    }
    // 🛡️ Signaux de confiance : TOUS les indicateurs de confiance consolidés
    // dans un seul endroit (visible par tous, pas réservé PRO — ça motive).
    html += trustStatsTiles(profile);
    box.innerHTML = html;
}

// Grille des signaux de confiance : note, missions accomplies, missions
// confirmées par des clients, temps de réponse, statut de vérification.
// Toutes les données existent déjà dans le profil (aucune lecture en plus).
function trustStatsTiles(profile) {
    profile = profile || {};
    const missions = profile.missions ? Object.keys(profile.missions).length : 0;
    const confirmed = profile.confirmedMissions ? Object.keys(profile.confirmedMissions).length : 0;
    const react = (profile.reactivity && (profile.reactivity.count || 0) >= 2) ? '~' + formatReactivity(profile.reactivity.min) : '—';
    const rating = profile.ratingCount ? (profile.ratingAvg || 0).toFixed(1) + '★' : '—';
    const status = profile.verified ? '🛡️' : (profile.phoneVerified ? '📱' : (profile.emailVerified ? '✉️' : '—'));
    const tile = (value, label, color) => `
      <div style="flex:1;min-width:70px;text-align:center;padding:10px 6px;background:rgba(255,255,255,0.04);border-radius:10px;">
        ...
        <div style="font-size:10px;color:var(--text-dim,#999);margin-top:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${label}</div>
      </div>`;
    return `
      <div style="font-weight:700;font-size:14px;color:var(--gold,#FFD700);margin:14px 0 8px;">${t('trustSignalsTitle')}</div>
      <div style="display:flex;flex-wrap:wrap;gap:8px;">
        ${tile(rating, t('statRatingLabel'), '#FFD700')}
        ${tile(String(missions), t('statMissionsLabel'), '#2D6CDF')}
        ${tile(String(confirmed), t('statConfirmedLabel'), '#2EA067')}
        ${tile(react, t('statReactivityLabel'), '#E88A2A')}
        ${tile(status, t('statStatusLabel'), 'var(--blue)')}
      </div>`;
}
...
function renderArtisanOfMonth() {
    ...
    if (!box) return;
    // Mode catégorie : quand un filtre de catégorie est actif, on met en avant
    // le meilleur artisan DE CETTE CATÉGORIE (catégorie = celle de sa dernière
    // annonce — les profils n'ont pas de catégorie structurée, on la déduit
    // d'allJobs). Sinon : champion global.
    const cat = (typeof currentCategory !== 'undefined' && currentCategory !== 'all') ? currentCategory : null;
    const catLatest = {};
    try {
        for (const j of allJobs) {
...
// ==========================================
// ===== CERTIFICATION DE COMPÉTENCE PAR MÉTIER (roadmap #10) =====
// Les missions consignées portent leur catégorie (icon). On déduit les
// SPÉCIALITÉS : 3+ missions dans la même catégorie = « certifié » dedans.
// Top 2 catégories (les plus fortes d'abord), max 2 badges pour ne pas
// noyer le profil. Zéro donnée nouvelle : tout vient de profile.missions.
const MISSION_CAT_EMOJI = { btp: '🏗️', electricite: '⚡', plomberie: '🔧', menage: '🧹', jardinage: '🌿', mecanique: '🔩', informatique: '💻' };
function missionCategoryCounts(profile) {
  const counts = {};
  try {
    const missions = (profile && profile.missions) || {};
    Object.values(missions).forEach(m => { if (m && m.icon) counts[m.icon] = (counts[m.icon] || 0) + 1; });
  } catch (e) {}
  return counts;
}
...
function computeAchievementBadges(profile) {
    ...
    // 🎓 Spécialités métier (roadmap #10) : top 2 catégories avec 3+ missions
    topMissionCategories(profile, 3, 2).forEach(([cat, n]) => {
      badges.push({ icon: MISSION_CAT_EMOJI[cat] || '🛠️', label: n + '+ missions ' + getCatLabel(cat), color: '#7A4EC2' });
    });
    return badges;
}
...
// Badges de confiance affichés sur le profil PUBLIC d'un artisan/entreprise :
// preuve sociale (identité vérifiée, PRO, excellence 4.5★+, clients servis,
// portfolio). Réutilise computeAchievementBadges() pour que le profil public
// et « Mes badges » du profil privé restent toujours cohérents.
...
function renderPublicMissions(profile) {
    ...
    const items = ids.map(id => ({ id, m: missions[id] || {} }))
        ...
        .slice(0, 3);
    const dateFmt = (ts) => { if (!ts) return ''; try { const d = new Date(ts); if (isNaN(d.getTime())) return ''; return d.toLocaleDateString(LANG_LOCALE[currentLang] || 'fr-FR', { day: 'numeric', month: 'short', year: 'numeric' }); } catch (e) { return ''; } };
    // 🎓 Spécialités métier (roadmap #10) : catégories avec 3+ missions
    const topCats = topMissionCategories(profile, 3, 3);
    const catsHtml = topCats.length ? `<div style="margin-top:10px;font-size:11px;font-weight:800;letter-spacing:0.5px;color:var(--text-dim,#999);">${t('specialtiesLabel')}</div>
      <div style="display:flex;flex-wrap:wrap;gap:6px;margin-top:5px;">
        ${topCats.map(([cat, n]) => `<span style="display:inline-flex;align-items:center;gap:4px;font-size:11px;font-weight:700;color:#7A4EC2;background:rgba(122,78,194,0.12);border:1px solid rgba(122,78,194,0.4);padding:4px 9px;border-radius:16px;">${MISSION_CAT_EMOJI[cat] || '🛠️'} ${escapeHtml(getCatLabel(cat))} ×${n}</span>`).join('')}
      </div>` : '';
...
// ===== COMPLETION DU PROFIL (meter de complétion, type LinkedIn) =====
// Calcule un % de complétion du profil + la liste des champs manquants.
// Motivation : un profil complet inspire confiance et reçoit plus de contacts.
function computeProfileCompleteness(profile) {
  profile = profile || {};
  const checks = [];
  checks.push({ done: !!profile.profileImage, weight: 30, label: t('changePhoto') });
  checks.push({ done: !!(profile.name || profile.company), weight: 25, label: t('fieldNameCompany') });
  checks.push({ done: !!profile.jobTitle, weight: 20, label: t('fieldJobTitle') });
  checks.push({ done: !!(profile.skills && String(profile.skills).trim()), weight: 15, label: t('fieldSkills') });
  checks.push({ done: !!(profile.whatsappNumber && String(profile.whatsappNumber).trim()), weight: 10, label: t('yourWhatsappNumberLabel') });
  const percent = Math.round(checks.reduce((s, c) => s + (c.done ? c.weight : 0), 0));
...
function renderProfileCompleteness(profile) {
  ...
  box.style.display = 'block';
  const color = percent >= 80 ? 'var(--green,#25D366)' : (percent >= 50 ? 'var(--gold,#FFD700)' : '#E5484D');
  ...
  box.innerHTML = `
    <div style="display:flex;justify-content:space-between;align-items:baseline;margin-bottom:6px;">
      <span style="font-weight:800;font-size:13px;color:var(--text,#fff);">📊 ${t('completenessTitle')}</span>
      <span style="font-weight:800;font-size:16px;color:${color};">${percent}%</span>
    </div>
    <div style="height:8px;background:rgba(255,255,255,0.08);border-radius:6px;overflow:hidden;">
      <div style="height:100%;width:${percent}%;background:${color};border-radius:6px;transition:width .4s;"></div>
    </div>
    ${missingHtml}`;
}
    ...
    if (String(cid).indexOf('__') !== -1) {
        ...
        if (window.W) {
            ...
            if (pUid && !W.contacts.some(c => c.id === pairId)) {
                ...
                W.contacts.push({ id: pairId, name: nm, avatar: avatarSrcFor(nm, (profilesCache[pUid] || {}).profileImage), online: false, lastSeen: '', about: '', profileType: (profilesCache[pUid] || {}).profileType || 'person' });
            }
            W.openChat(pairId);
        }
        return;
...
function attachChatListeners(cid, peerUid) {
    ...
    chatMsgsRef.on('value', (snap) => {
        ...
        // Dernier message de l'autre partie → proposer les réponses IA
        if (typeof updateAiSuggestRow === 'function') updateAiSuggestRow();
    });
    if (peerUid) {
        ...
        chatPresenceRef.on('value', (snap) => {
            ...
            if (c) {
                ...
                c.lastSeen = p && p.lastChanged ? timeAgo(p.lastChanged) : ''; // ⚠ pas de préfixe "il y a " codé en dur : le widget applique lastSeen "vu {x}"/"last seen {x}" et timeAgo() est déjà traduit
...
function mapWidgetMsg(key, m, cid) {
    ...
    if (m.imageUrl) { out.image = chatPhotoDisplayUrl(m.imageUrl); out.imageFull = m.imageUrl; }
    if (m.images && m.images.length) { out.images = m.images.map(chatPhotoDisplayUrl); out.imagesFull = m.images; }
    if (m.autoReply) out.autoReply = true;
...
// Le job lié à la conversation OUVERTE vient de changer (ex : l'employeur a
// marqué « pourvu » en temps réel) : on re-rend l'action de confirmation pour
// que l'artisan voie le bouton APPARAÎTRE sans fermer/rouvrir le chat.
async function refreshJobBarConfirmForOpenChat(jobId) {
    try {
        ...
        if (prev) prev.remove();
        if (typeof renderJobBarConfirmAction === 'function') await renderJobBarConfirmAction(jobId);
    } catch (e) {}
}
...
async function renderJobBarConfirmAction(jobId) {
    ...
    try {
        ...
        if (snap.exists()) {
            ...
            b.style.cssText = 'margin-left:8px;flex-shrink:0;font-size:12px;font-weight:800;color:#2EA067;background:rgba(46,160,103,.15);border:1px solid rgba(46,160,103,.5);white-space:nowrap;padding:6px 10px;border-radius:10px;';
        ...
        btn.style.cssText = 'margin-left:8px;flex-shrink:0;font-size:12px;font-weight:800;background:linear-gradient(135deg,#2EA067,#1f8a4e);border:none;color:#fff;padding:8px 14px;border-radius:10px;cursor:pointer;box-shadow:0 2px 10px rgba(46,160,103,.45);';
...
async function confirmMissionFromChat(jobId) {
    ...
    try {
        ...
        // 🔔 Notifier l'EMPLOYEUR (propriétaire du job) via le worker dédié —
        // silencieux et non bloquant : la confirmation est déjà persistée,
        // le push est un bonus (sauté si le worker n'est pas configuré).
        try {
            if (job.user && job.id) {
                const token = await me.getIdToken();
                notifyDirect({ event_type: 'mission-confirmed', idToken: token, clientUid: job.user, jobId: job.id, jobTitle: job.title || '' });
            }
        } catch (ne) { console.warn('mission notify error', ne); }
...
function quickRepliesFor(context, job) {
    if (context === 'job') {
        // Annonce POURVUE → mission terminée : les chips de coordination
        // (prix, horaires…) n'ont plus de sens → chips de remerciement.
        if (job && job.status === 'filled') {
            return [t('qrMissionThanks'), t('qrRecommend'), t('qrThanks')];
        }
        const list = [t('qrAvailable')];
        // Chip prix CONTEXTUELLE : porte le VRAI prix de l'annonce (ou
        // « gratuit ») — la coordination part sur des chiffres réels.
        if (job && !isFreePrice(job)) list.push(t('qrPriceContext').replace('{price}', priceLabel(job)));
        else if (job) list.push(t('qrPriceFree'));
        else list.push(t('qrPrice'));
        list.push(t('qrWhen'), t('qrOnWay'), t('qrGo'), t('qrThanks'));
        return list;
    }
    return [t('qrHello'), t('qrAvailable'), t('qrAgree'), t('qrThanks'), t('qrCall')];
}
...
// ============================================================
// SUGGESTIONS DE RÉPONSE IA DANS LE CHAT (20260905g)
// ============================================================
// L'artisan occupé reçoit 1-2 réponses professionnelles PRÊTES À
// ENVOYER, rédigées par l'IA dans SA voix (métier, compétences,
// disponibilité, annonce liée en contexte). Un tap insère le texte
// dans la saisie — l'artisan reste libre de l'éditer avant d'envoyer.
// IA indisponible → 1 réponse de repli par template (jamais le vide).
let aiSuggestBusy = false;
function renderAiIdleChip() {
  ...
  chip.textContent = t('aiSuggestBtn');
  chip.addEventListener('click', () => generateAiSuggestion());
  row.appendChild(chip);
}
// Affiche la ligne « ✨ » quand le DERNIER message de la conversation
// active vient de l'autre partie (c'est à moi de répondre).
function updateAiSuggestRow() {
  ...
  if (!open) { box.classList.remove('active'); return; }
  const msgs = (W.convs[W.activeId] || []);
  const last = msgs[msgs.length - 1];
  if (last && last.from !== 'me') {
    box.classList.add('active');
    if (!box.dataset.rendered) { renderAiIdleChip(); box.dataset.rendered = '1'; }
  } else {
...
async function generateAiSuggestion() {
  const box = document.getElementById('waAiSuggest');
  const row = document.getElementById('waAiRow');
  if (!box || !row || !window.W || !W.activeId || aiSuggestBusy) return;
  const me = auth.currentUser;
  if (!me || me.isAnonymous) return;
  aiSuggestBusy = true;
  row.innerHTML = '';
  ...
  try {
    // Contexte : les 6 derniers messages (rôle : texte), mon profil,
    // l'annonce liée à la conversation (bannière du job).
    const msgs = (W.convs[W.activeId] || []).slice(-6).map(m =>
      (m.from === 'me' ? 'Moi' : 'Client') + ' : ' +
      (m.text || (m.image ? '📷 Photo' : (m.images && m.images.length ? '📷 Photos' : '')))
    ).filter(s => s.split(' : ')[1]);
    const profile = profilesCache[me.uid] || {};
    let jobCtx = '';
    const jobBar = document.getElementById('waJobBar');
    if (jobBar && jobBar.dataset.jobId && jobBar.dataset.jobId !== 'general') {
      const job = ((typeof jobsById !== 'undefined' && jobsById[jobBar.dataset.jobId]) ||
        (typeof allJobs !== 'undefined' ? allJobs.find(j => j.id === jobBar.dataset.jobId) : null));
      if (job) jobCtx = String(job.title || '') + ' — ' + String(job.desc || '').slice(0, 300);
    }
    let suggestions = [];
    try {
      const controller = new AbortController();
      ...
      const res = await fetch(CHAT_PROXY_URL, {
        ...
        body: JSON.stringify({
          ...
          jobContext: jobCtx,
          lang: profile.lang || currentLang
        }),
        signal: controller.signal
      });
      clearTimeout(timeout);
      const data = await res.json().catch(() => ({}));
      if (!data.fallback && Array.isArray(data.suggestions)) {
        suggestions = data.suggestions.map(s => String(s).trim()).filter(Boolean).slice(0, 2);
      }
    } catch (e) { /* IA indisponible → repli ci-dessous */ }
    if (!suggestions.length) {
      suggestions = [t('aiSuggestFallback').replace('{name}', String(profile.name || profile.company || t('userFallback')).slice(0, 40))];
    }
    row.innerHTML = '';
    suggestions.forEach(s => {
      ...
      chip.addEventListener('click', () => {
        const txt = document.getElementById('waTxt');
        if (txt) {
          txt.value = (txt.value && !/[s,]$/.test(txt.value) ? txt.value + ' ' : txt.value) + s;
          if (W.autoResize) W.autoResize(txt);
          txt.focus();
        }
        // retour au bouton ✨ (l'artisan peut relancer une nouvelle suggestion)
...
function updateQuickReplies() {
    ...
    // Chips CONTEXTUELLES : si l'annonce en bannière change de statut/prix
    // (ex. l'artisan la marque « pourvu » pendant la discussion), les chips
    // se recalculent (signature sur statut + prix).
    const jid = jobBar ? jobBar.dataset.jobId : null;
    const job = (ctx === 'job' && jid && jid !== 'general')
        ? ((typeof jobsById !== 'undefined' && jobsById[jid]) || (typeof allJobs !== 'undefined' ? allJobs.find(j => j.id === jid) : null))
        : null;
    const sig = ctx + (job ? '|' + (job.status || '') + '|' + (job.price || '') : '');
    if (sig !== quickRepliesSig) { quickRepliesSig = sig; renderQuickReplies(ctx, job); }
...
function openUserChat(peerUid, jobId, peerName, jobTitle, prefillMsg) {
    ...
    if (window.W) {
        ...
        if (!existing) {
            ...
            W.contacts.push({ id: tid, name: name, avatar: avatarSrcFor(name, (profilesCache[peerUid] || {}).profileImage), online: false, lastSeen: '', about: jobTitle || '', profileType: (profilesCache[peerUid] || {}).profileType || 'person' });
            W.renderChats();
        }
        W.openChat(tid);
        ...
        if (jobId && jobId !== 'general' && jobTitle) {
          ...
          if (input && !input.value && (W.convs[tid] || []).length === 0) {
            const staticMsg = prefillMsg || t('chatFirstMsgJob').replace('{jobTitle}', String(jobTitle).slice(0, 80));
            ...
            // Mise à niveau IA du message par défaut (pas pour un prefillMsg
            // explicite comme la demande de devis, qui a déjà son propre
            // message dédié) : le chat s'ouvre tout de suite avec le
            // message-modèle générique, puis l'IA le remplace dès qu'elle
            // est prête par une version qui relie vraiment les compétences
            // de l'artisan au besoin décrit dans CETTE annonce — même
            // principe « affichage immédiat, amélioration en arrière-plan »
            // que la traduction du popup carte (voir translateMapPopupIfNeeded).
            if (!prefillMsg) upgradeChatPrefillWithAI(tid, jobId, input, staticMsg);
          }
        }
    }
}
...
function patchChatWidgetForFirebase() {
    ...
    W.sendMessage = function(cid, data) {
        ...
        if (data.images && data.images.length) tempMsg.images = data.images.slice(0, 6);
        ...
        const isDataImage = !!(data.image && String(data.image).startsWith('data:')) || !!(data.images && data.images.length);
        ...
        if (isDataImage) {
            sendChatPhotoToThread(cid, tempMsg, data.images || [data.image], tempMsg.__reply || undefined, clearPending);
        } else {
            const payload = {};
    ...
    // ---- SUPPRIMER EN BLOC (multi-sélection) : confirmation UNIQUE, pas un
    // dialog par message — même sémantique que la suppression simple :
    // « pour moi » (deletedFor) par défaut, « pour tout le monde » (deleted)
    // seulement pour les messages RÉCENTS (≤ 15 min) envoyés par moi, et
    // seulement si l'utilisateur a confirmé (la règle Firebase l'applique).
    W.deleteMessages = function(cid, mids) {
        const me = auth.currentUser;
        const ms = this.convs[cid] || [];
        const sel = (mids || []).map(id => ms.find(x => x.id === id)).filter(Boolean);
        if (!sel.length) return;
        if (!confirm(t('delManyConfirm').replace('{x}', sel.length))) return;
        const mineRecent = sel.filter(m => m.from === 'me' && (Date.now() - (m._ts || 0)) < 15 * 60 * 1000);
        let forEveryone = false;
        if (mineRecent.length) forEveryone = confirm(t('delManyAllConfirm'));
        sel.forEach(m => {
            origDeleteMessage.call(this, cid, m.id);
            if (me && m.fbId) {
                const ref = db.ref('chats/' + cid + '/messages/' + m.fbId);
                if (forEveryone && m.from === 'me') ref.child('deleted').set(true).catch(() => {});
                else ref.child('deletedFor/' + me.uid).set(true).catch(() => {});
            }
        });
    };
    ...
    // ---- Transfert d'un ou PLUSIEURS messages vers une autre conversation ----
    // (multi-sélection : this._fwdList ; simple : this._fwdMsg)
    W._doFwd = function(cid) {
        const toCid = cid;
        this.closeFwd();
        if (!toCid || toCid === this.activeId) return;
        const list = this._fwdList && this._fwdList.length ? this._fwdList : (this._fwdMsg ? [this._fwdMsg] : []);
        list.forEach(src => {
            if (!src) return;
            const data = {};
...
async function sendChatMessageToThread(threadId, payload, onDone) {
    ...
    try {
        ...
        const preview = payload.images ? '📷 Photos' : (payload.imageUrl ? '📷 Photo' : (payload.audioUrl ? '🎤 ' + t('voiceMsg') : (payload.text || '').slice(0, 100)));
        ...
        if (payload.images) msg.images = payload.images;
        if (payload.autoReply) msg.autoReply = true;
...
// Envoi d'une photo OU PLUSIEURS photos venues du widget (data URLs) :
// compression éventuelle, upload Cloudinary (séquentiel, l'ordre est
// conservé), puis écriture Firebase avec les URL finales. 1 photo =
// imageUrl (compatible avec les anciens messages), n photos = images[...].
async function sendChatPhotoToThread(threadId, tempMsg, dataUrls, replyTo, onDone) {
    try {
        ...
        // la bulle optimiste (data:URL locale) passe aux URL Cloudinary
        if (window.W) {
            const conv = W.convs[threadId] || [];
            const m = conv.find(x => x.id === tempMsg.id);
            if (m) {
                if (urls.length === 1) { m.image = urls[0]; m.images = null; }
                else { m.images = urls; m.image = null; }
                if (W.activeId === threadId && W.renderMsgs) W.renderMsgs();
            }
        }
        const payload = {};
        if (urls.length === 1) payload.imageUrl = urls[0];
        else payload.images = urls;
...
// ============================================================
// ===== AUTO-RÉPONSE IA (20260905f) =====
// Quand un message arrive alors que JE ne regarde PAS l'app (onglet en
// arrière-plan), l'IA répond en mon nom, clairement étiquetée « Réponse
// automatique » — le client a le sentiment d'un artisan réactif, et la
// conversation ne meurt pas pendant que je suis absent.
// Anti-boucle (triple) : on ne répond JAMAIS à une auto-réponse, 1
// auto-réponse max par conversation / 4 h (localStorage), et seulement si
// le message vient bien de l'autre partie. IA indisponible → message
// d'absence classique (il y a toujours une réponse, jamais le silence).
let aiAwayInFlight = false;
const AI_AWAY_COOLDOWN_MS = 4 * 3600 * 1000;
async function maybeAiAwayReply(threadId) {
    if (aiAwayInFlight) return;
    try {
        if (document.visibilityState === 'visible') return; // je regarde l'app : je réponds moi-même
        const me = auth.currentUser;
        if (!me || me.isAnonymous) return;
        try { if (Date.now() - (Number(localStorage.getItem('aiAway_' + threadId)) || 0) < AI_AWAY_COOLDOWN_MS) return; } catch (e) {}
        // Le DERNIER message de la conversation : il vient de l'autre partie
        // et n'est pas lui-même une auto-réponse (sinon boucle infinie).
        const snap = await db.ref('chats/' + threadId + '/messages').limitToLast(1).once('value');
        ...
        const peerUid = peerUidOf(threadId);
        if (!peerUid) return;
        // Contexte : l'annonce liée à la conversation (si elle en a une)
        let jobCtx = '';
        try {
            ...
            if (meta.jobTitle) {
                ...
                jobCtx = String(job ? (job.title || '') + ' — ' + (job.desc || '') : meta.jobTitle).slice(0, 500);
            }
        } catch (e) {}
        let text = '';
        try {
            ...
            const res = await fetch(CHAT_PROXY_URL, {
                ...
                body: JSON.stringify({
                    mode: 'ai_away_reply',
                    incomingMessage: String(last.text || (last.imageUrl || (last.images && last.images.length) ? '📷 Photo' : '')).slice(0, 600),
                    proTitle: profile.jobTitle || '',
                    proSkills: profile.skills || '',
                    availability: profile.availability || '',
                    jobContext: jobCtx,
            ...
            clearTimeout(timeout);
            const data = await res.json().catch(() => ({}));
            if (!data.fallback && typeof data.message === 'string' && data.message.trim()) text = data.message.trim().slice(0, 500);
        } catch (e) { /* IA indisponible → repli ci-dessous */ }
        if (!text) text = t('aiAwayFallback').replace('{name}', String(profile.name || profile.company || t('userFallback')).slice(0, 40));
        try { localStorage.setItem('aiAway_' + threadId, String(Date.now())); } catch (e) {}
        await sendChatMessageToThread(threadId, { text: text, autoReply: true }, null);
    } catch (e) { console.warn('aiAwayReply error', e); }
    finally { aiAwayInFlight = false; }
}
...
function startInboxBadgeWatch(uid) {
    ...
    inboxRef.on('value', (snap) => {
        ...
        // Auto-réponse IA : NOUVEAU message de l'autre partie alors que
        // l'onglet est en arrière-plan (les conditions internes sont plus
        // strictes : anti-boucle, cooldown 4 h, jamais au premier plan).
        for (const [tid, e] of entries) {
          const prev = (lastInboxSnapshot || []).find(([t2]) => t2 === tid);
          if (e && e.lastFrom && e.lastAt > ((prev && prev[1] && prev[1].lastAt) || 0)) maybeAiAwayReply(tid);
        }
...
// Remplace le message de premier contact pré-rempli (statique) par la
// version IA dès qu'elle est prête, UNIQUEMENT si rien n'a changé entre-
// temps : l'utilisateur pourrait avoir déjà commencé à taper, corrigé le
// texte, vidé le champ volontairement, ou même déjà envoyé le message
// pendant les quelques secondes d'appel IA — dans tous ces cas, on
// n'écrase RIEN (jamais remplacer ce qu'une personne a délibérément fait).
// Fail-open comme craftContactMessage() elle-même : en cas d'erreur/lenteur
// IA, on ne fait simplement rien, le message-modèle déjà affiché reste
...
async function upgradeChatPrefillWithAI(tid, jobId, input, staticMsg) {
  ...
  try {
    ...
    return; // pas de profil lisible → on garde le message-modèle, rien à faire
  }

  const message = await craftContactMessage(job, profile);
  if (!message || message === staticMsg) return; // rien de mieux à proposer

  // Toujours le même input ? (le widget peut recréer le DOM en changeant
  // de conversation entre-temps) — sinon, retrouver l'input actuel serait
  // risqué (pourrait appartenir à une autre conversation déjà).
  const stillSameInput = document.getElementById('waTxt') === input;
  if (!stillSameInput) return;
  // Le champ contient-il encore EXACTEMENT le message-modèle initial, et la
  // conversation n'a-t-elle toujours aucun message envoyé ?
  if (input.value !== staticMsg) return;
  if ((W.convs[tid] || []).length !== 0) return;
...
async function getReviewsSummary(ownerUid, comments) {
  ...
  try {
    const cached = await db.ref(`reviewSummaries/${ownerUid}/${cacheKey}`).once('value');
  ...
  try {
    ...
    db.ref(`reviewSummaries/${ownerUid}/${cacheKey}`).set({ text: data.summary }).catch(() => {});
    return data.summary;
  } catch (err) {
...
async function renderMyReviewAction(job) {
    ...
    try {
        ...
        const open = entries
            .filter(e => e.val && e.val.contactUid !== user.uid && !e.val.reviewed && !reviewShouldSilence(e.key))
        ...
        if (!open.length) {
            // « déjà noté » = flag serveur OU marquage local (l'avis a été
            // envoyé mais le flag peut ne pas être propagé) — sinon le bouton
            // reviendrait et renverrait « rate_limited » (boucle d'erreur).
            const anyReviewed = entries.some(e => e.val && e.val.contactUid !== user.uid && (e.val.reviewed || reviewLocalGet('reviewDone_', e.key)));
...
// ===== ANTI-BOUCLE D'AVIS =====
// Bug corrigé : le prompt « Comment ça s'est passé ? » revenait EN BOUCLE
// suivi d'une erreur. Causes :
//  (1) le flag reviewed=true posé par le worker est ASYNCHRONE — entre la
//      soumission réussie et sa propagation, tout nouvel événement auth
//      relançait checkPendingReviews → re-prompt pour le même contact ;
//  (2) si le worker créait l'avis mais échouait à poser le flag, l'avis
//      EXISTAIT : le nouvel envoi renvoyait « rate_limited » (cooldown 30 j)
//      → erreur visible + prompt qui revenait indéfiniment ;
//  (3) un « skip » en hors-ligne (écriture refusée) reproposait le prompt à
//      chaque chargement, sans aucun délai.
// Correctif : marqueurs localStorage + set de session. Aucun changement de
// worker ni de règles — le serveur reste la source de vérité, ces marqueurs
// sont un filet anti-folie côté client.
const reviewPromptedThisSession = new Set();
// Hook de tests (jsdom) : les bindings const ne sont pas accessibles depuis
// l'extérieur du script (comme allJobs/profilesCache avant l'alias).
try { window.__reviewPromptedSession = reviewPromptedThisSession; } catch (e) {}
function reviewLocalGet(prefix, contactKey) {
  try { return Number(localStorage.getItem(prefix + contactKey)) || 0; } catch (e) { return 0; }
}
function reviewLocalSet(prefix, contactKey) {
  try { localStorage.setItem(prefix + contactKey, String(Date.now())); } catch (e) {}
}
// Ce contact ne doit plus être reproposé : avis déjà envoyé (ou refusé car
// un avis existait déjà), skip local récent (24 h) ou échec récent (6 h).
function reviewShouldSilence(contactKey) {
  const now = Date.now();
  if (reviewLocalGet('reviewDone_', contactKey)) return true;
  // pas de marqueur = 0 → now - 0 est énorme → jamais dans la fenêtre
  if (now - reviewLocalGet('reviewSkip_', contactKey) < 24 * 3600 * 1000) return true;
  if (now - reviewLocalGet('reviewErr_', contactKey) < 6 * 3600 * 1000) return true;
  return false;
}
  ...
  try {
    ...
    for (const jid of jobIds) {
      ...
      entries.forEach(entry => {
        const data = entry.val;
        // Filtre anti-boucle : silences locaux + déjà prompté cette session
        // (le flag serveur met quelques secondes à se propager après un envoi).
        if (data && data.contactUid !== uid && data.reviewed === false && (Date.now() - data.timestamp) > REVIEW_DELAY_MS
            && !reviewShouldSilence(entry.key) && !reviewPromptedThisSession.has(entry.key)) {
    ...
    showReviewPrompt(pendingKey, uid, jobTitle, contactorName, pending.contactUid);
  } catch (e) {
    console.warn('checkPendingReviews error', e);
  }
}

// providerUid (5e param) = l'ARTISAN noté (contactUid du contact) : utilisé
// pour la notification « nouvel avis reçu ». reviewerUid (2e) = celui qui
// note (l'employeur) — le param s'appelait « jobOwnerUid » avant l'inversion
// de direction des avis (30/08), d'où le renommage.
function showReviewPrompt(contactKey, reviewerUid, jobTitle, ownerName, providerUid) {
  if (document.getElementById('reviewPromptOverlay')) return; // déjà affiché
  reviewPromptedThisSession.add(contactKey); // anti re-prompt dans la session
  ...
  overlay.querySelector('#reviewSkipBtn').onclick = async () => {
    // Filet anti-boucle : même si l'écriture serveur échoue (hors-ligne), le
    // prompt ne reviendra pas avant 24 h (au lieu de chaque rechargement).
    reviewLocalSet('reviewSkip_', contactKey);
    try {
      await db.ref('job_contacts/' + contactKey + '/reviewed').set(true);
    } catch (e) {
      console.warn('Skip avis: écriture échouée (re-proposé dans 24 h max)', e);
    }
    overlay.remove();
  };
    ...
    try {
      ...
      if (!res.ok || !data.success) {
        ...
        if (data.error === 'rate_limited') {
          // Un avis pour cet artisan EXISTE déjà (souvent créé lors d'un
          // envoi précédent dont le flag n'a pas propagé) : on ne re-tente
          // JAMAIS — avant c'était la boucle « erreur → re-prompt → erreur ».
          ...
          reviewLocalSet('reviewErr_', contactKey); // backoff 6 h sur les re-prompts auto
      ...
      // Avis accepté : marquage local anti-boucle (le flag serveur
      // reviewed=true met quelques secondes à se propager — sans ce
      // marquage, un nouvel événement auth relançait le prompt).
      reviewLocalSet('reviewDone_', contactKey);

      triggerInstantNotify('new-review'); // filet de sécurité (cron) — le push direct ci-dessous est instantané

      // 🔔 Notifier l'ARTISAN noté (worker de notifs directes) : « Nouvel
      // avis reçu ⭐ 4/5 ». Le worker re-vérifie que l'avis existe vraiment
      // et qu'il a été laissé par cet appelant (anti-faux-push).
      try {
        const notifiedProvider = providerUid || (contactKey.split('_')[1] || '');
        if (notifiedProvider) {
          notifyDirect({ event_type: 'new-review', idToken, providerUid: notifiedProvider, reviewId: data.reviewId, rating: selectedRating });
        }
      } catch (ne) { console.warn('review notify error', ne); }
      ...
      // On recalcule le profil de l'ARTISAN noté (providerUid) — avant
      // l'inversion de direction c'était le « jobOwnerUid » (même personne).
      const ratingTarget = providerUid || (contactKey.split('_')[1] || '');
      if (ratingTarget) syncOwnerRating(ratingTarget);
      ...
      reviewLocalSet('reviewErr_', contactKey); // backoff 6 h (réseau instable → pas de boucle)
      showToast(t('reviewSendError'), 'error');
      submitBtn.disabled = false;
      submitBtn.textContent = t('reviewSendBtn');
    }
...
syncJobs();
syncProfilesCache();
if (typeof refreshInstallAppBtn === 'function') refreshInstallAppBtn(); // libellé du bouton d'installation selon le mode
...
function toggleSettingsPanel() {
  ...
  if (opening) {
    ...
    if (typeof refreshInstallAppBtn === 'function') refreshInstallAppBtn(); // libellé selon le mode (installée ou non)
  }
}

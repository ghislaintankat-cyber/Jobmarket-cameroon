...
// copier. Il reste utile comme frein basique (empêche un scan random de
// l'URL), mais la vraie protection contre l'abus, c'est la limite de
// fréquence ci-dessous : même secret en main, personne ne peut faire partir
// plus d'UN déclenchement GitHub toutes les MIN_INTERVAL_MS.
const MIN_INTERVAL_MS = 20_000; // 20s : largement assez rapide comparé au cron de 15 min, mais borne l'abus et coalesce les publications rapprochées en un seul run.
...
export default {
  async fetch(request, env) {
    ...
    // indisponible pour une raison quelconque, on continue sans limiter
    // plutôt que de bloquer la fonctionnalité — le pire cas reste "un
    // déclenchement GitHub en trop", jamais "notification perdue".
    if (env.RATE_LIMIT_KV) {
      try {
        const last = await env.RATE_LIMIT_KV.get('last-dispatch');
        const now = Date.now();
        if (last && (now - Number(last)) < MIN_INTERVAL_MS) {
          return new Response('Coalesced (rate-limited) — un déclenchement récent couvre déjà ce job', { status: 200 });
        }
    ...
    try {
      ...
      if (!ghRes.ok) {
        ...
        return new Response('GitHub dispatch failed, cron de secours prendra le relais', { status: 502 });
      }
      return new Response('OK', { status: 200 });
    } catch (err) {
      console.error('Worker error', err);
      return new Response('Erreur relais, cron de secours prendra le relais', { status: 500 });
    }
  }
};
